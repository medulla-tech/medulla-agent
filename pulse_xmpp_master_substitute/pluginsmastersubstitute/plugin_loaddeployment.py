    ...
    try:
        ...
        try:
            ...
            if not self.mycompteurcallplugin % 6:
                list_ars_syncthing_pause = (
                    XmppMasterDatabase().get_ars_for_pausing_syncthing()
                )
        ...
        for deployuuid in self.machineDeploy:
            try:
                ...
                logging.debug(
                    "Sending deployment on machine %s package %s"
        ...
        for machine in machines_waiting_online:
            ...
            try:
                ...
                if XmppMasterDatabase().getPresenceuuid(machine["inventoryuuid"]):
                    machine_hostname = machine["jidmachine"].split("@")[0][:-4]
                    msg = (
                        "Machine %s is online. Starting the deployment"
                        % machine_hostname
                    )
                    self.xmpplog(
                        msg,
                    ...
                    if (
                        ...
                        try:
                            if "jidmachine" in data and data["jidmachine"] != "":
                                checkChangedJID = (
                                    XmppMasterDatabase().update_jid_if_changed(
                                        data["jidmachine"]
                                    )
                                )
                                if checkChangedJID:
                                    if checkChangedJID[0]["jid"] != data["jidmachine"]:
                                        logging.warning(
                                            "Machine JID changed since creation of deployment"
                                        )
                                        logging.warning(
                                            "Machine JID %s -> %s"
                                            % (
                                                data["jidmachine"],
                                                checkChangedJID[0]["jid"],
                                            )
                                        )
                                        logging.warning(
                                            "Relay server JID %s -> %s"
                                            % (
                                                data["jidrelay"],
                                                checkChangedJID[0]["groupdeploy"],
                                            )
                                        )
                                        msglog.append(
                                            "jid machine changed : replace jid mach from %s to %s"
                                            % (
                                                data["jidmachine"],
                                                checkChangedJID[0]["jid"],
                                            )
                                        ...
                                        data["jidrelay"] = checkChangedJID[0][
                                            "groupdeploy"
                                        ]
                                        XmppMasterDatabase().replace_jid_mach_ars_in_deploy(
                                            data["jidmachine"],
                                            data["jidrelay"],
                                            data["title"],
                                        )
...
def syncthingdeploy(self):
    ...
    if len(iddeploylist) != 0:
        for iddeploy in iddeploylist:
            logging.debug(
                "We correctly initialized the synching deployment for the group: %s"
                % iddeploy
            )
        ...
        logging.debug(
            "This is not a syncthing deployment, so we did not initialize it."
        )
...
def send_session_command(
    ...
    if action is not None:
        ...
        if jid == self.boundjid.bare:
            self.callpluginsubstitute(action, data, sessionid=command["sessionid"])
        else:
...
def _sendwolgroup(self, listorset, nb=5000):
    ...
    try:
        ...
        for listsend in listforsplit:
            self.callpluginsubstitute("wakeonlangroup", {"macadress": list(listsend)})
    except Exception:
        logger.error("%s" % (traceback.format_exc()))
...
def read_conf_loaddeployment(objectxmpp):
    ...
    foldersession = os.path.abspath(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "..", "sessiondeploysubstitute"
        )
    )

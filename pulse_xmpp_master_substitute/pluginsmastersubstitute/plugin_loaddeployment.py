...
def scheduledeploy(self):
    ...
    if not self.process_load_deployment_on:
        logger.warning(
            "We cannot start a new deployment cyle. The previous one is still running.")
        return
    else:
        logger.debug(
            "We start a new deployment cycle of %s computers. Next check in %s seconds" %
            (self.deployment_nbr_mach_cycle, self.deployment_scan_interval))
    try:
        ...
        try:
            ...
            ).deployxmpp(limitnbr=self.deployment_nbr_mach_cycle)
        ...
        if nb_machine_select_for_deploy_cycle:
            ...
            logger.info(
                "scheduledeploy : mach %s time %s t/m %s" %
                (nb_machine_select_for_deploy_cycle,
                 timef,
                 (timef /
                  nb_machine_select_for_deploy_cycle) if nb_machine_select_for_deploy_cycle else "--"))
...
def scheduledeployrecoveryjob(self):
    ...
    wol_set = set()
    try:
        # We set the deploiement as ABORT ON TIMEOUT as the deploy launch
        # window is over.
        ...
        for machine in machines_scheduled_deploy:
            ...
            if resultpresence[UUID][1] == 0:
                ...
                # et la machine n'existe plus. soit son uuid GLPI a changer, ou
                # elle a ete suprimer. la machine n'existe plus.
                msglog.append(
        ...
        msglog = []
        # We search all the machines that was offline (deploy with state
        ...
        for machine in machines_waiting_online:
            logger.info(
                "Restarting the deploiement %s actually in  machines_waiting_online state" %
                machine['sessionid'])
            try:
                ...
                if XmppMasterDatabase().getPresenceuuid(
                    ...
                    # We need to check if there is a syncthing group. Then we
                    # can decide to add it.
                    if 'grp' in data['advanced'] and data['advanced']['grp'] is not None and \
                        ...
                        datasession = self.sessiondeploysubstitute.sessiongetdata(
                            machine['sessionid'])
                        msglog.append(
                        ...
                        if 'syncthing' in data['advanced'] and \
                            ...
                            self.xmpplog(
                                "<span class='log_warn'>There are not enough "
                ...
                if 'sessionid' in machine:
                    XmppMasterDatabase().replaydeploysessionid(
                        machine['sessionid'],
    ...
    try:
        ...
        if 'error' in objmachine and objmachine['error'] == "MultipleResultsFound":
            logger.warn(
                ...
                objmachine['error'])
            dupplicate_machines = XmppMasterDatabase(
            ).get_machine_with_dupplicate_uuidinventory(uuidmachine, None)
            logger.warn(
                'get_machine_with_dupplicate_uuidinventory %s' %
                dupplicate_machines)
            grparray = []
            jidarray = []
            keysyncthingarray = []
...
def syncthingdeploy(self):
    ...
    if len(iddeploylist) != 0:
        for iddeploy in iddeploylist:
            logging.debug(
                "We correctly initialized the synching deploy for the group: %s" %
                iddeploy)
        ...
        logging.debug(
            "This is not a syncthing deploy, so we did not initialize it.")
...
def read_conf_loaddeployment(objectxmpp):
    ...
    if not os.path.isfile(pathfileconf):
        ...
        if Config.has_option("parameters", "wol_interval"):
            objectxmpp.wol_interval = Config.getint(
                'parameters', 'wol_interval')
        else:
            objectxmpp.wol_interval = 60

        if Config.has_option("parameters", "deployment_scan_interval"):
            objectxmpp.deployment_scan_interval = Config.getint(
                'parameters', 'deployment_scan_interval')
        else:
            objectxmpp.deployment_scan_interval = 10

        if Config.has_option("parameters", "deployment_nbr_mach_cycle"):
            objectxmpp.deployment_nbr_mach_cycle = Config.getint(
                'parameters', 'deployment_nbr_mach_cycle')
        else:
            objectxmpp.deployment_nbr_mach_cycle = 100

        if Config.has_option("parameters", "deployment_end_timeout"):
            objectxmpp.deployment_end_timeout = Config.getint(
                'parameters', 'deployment_end_timeout')
        else:
            objectxmpp.deployment_end_timeout = 300

        if Config.has_option("parameters", "session_check_interval"):
            objectxmpp.session_check_interval = Config.getint(
                'parameters', 'session_check_interval')
        else:
            objectxmpp.session_check_interval = 15

        if Config.has_option("parameters",
                             "recover_glpi_identifier_from_name"):
            objectxmpp.recover_glpi_identifier_from_name = Config.getboolean(
                'parameters', 'recover_glpi_identifier_from_name')
        else:
            objectxmpp.recover_glpi_identifier_from_name = False

        if Config.has_option("parameters", "force_redeploy"):
            objectxmpp.force_redeploy = Config.getboolean(
                'parameters', 'force_redeploy')
        else:
            objectxmpp.force_redeploy = 1

        if Config.has_option("parameters", "reschedule"):
            objectxmpp.reschedule = Config.getboolean(
                'parameters', 'reschedule')
        else:
            objectxmpp.reschedule = 0

    # initialisation des object for deployement

    objectxmpp.applicationdeployjsonUuidMachineAndUuidPackage = types.MethodType(
        applicationdeployjsonUuidMachineAndUuidPackage, objectxmpp)

    objectxmpp.applicationdeployjsonuuid = types.MethodType(
        applicationdeployjsonuuid, objectxmpp)
    objectxmpp.applicationdeploymentjson = types.MethodType(
        applicationdeploymentjson, objectxmpp)
    ...
    objectxmpp.callpluginsubstitute = types.MethodType(
        callpluginsubstitute, objectxmpp)

    objectxmpp.directcallplugin = types.MethodType(
        directcallplugin, objectxmpp)

    objectxmpp.createsessionfordeploydiffered = types.MethodType(
        createsessionfordeploydiffered, objectxmpp)

    objectxmpp.send_session_command = types.MethodType(
        send_session_command, objectxmpp)
    ...
    objectxmpp.scheduledeployrecoveryjob = types.MethodType(
        scheduledeployrecoveryjob, objectxmpp)
    ...
    objectxmpp.handlemanagesession = types.MethodType(
        handlemanagesession, objectxmpp)

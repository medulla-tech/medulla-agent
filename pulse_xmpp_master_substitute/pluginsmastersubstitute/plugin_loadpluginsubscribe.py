...
def action(objectxmpp, action, sessionid, data, msg, dataerreur):
    ...
    if compteurcallplugin == 0:
        ...
        objectxmpp.add_event_handler(
            'changed_status', objectxmpp.changed_status)

        XmppMasterDatabase().update_enable_for_agent_subscription(
            objectxmpp.boundjid.bare)  # update down machine substitute manage by self agent
    ...
    try:
        ...
        if presence['type'] == 'unavailable':
            if lastevent and lastevent[0]['status'] == 1:
                XmppMasterDatabase().setUptime_machine(hostname, spresence, status=0,
                                                       updowntime=time.time() - lastevent[0]['time'], date=None)
            ...
            try:
                ...
                if "type" in result and result['type'] == "relayserver":
                    ...
                    logger.debug(
                        "Notify to all ARS, offline ARS %s" %
                        spresence)
            ...
            if lastevent:
                if lastevent[0]['status'] == 0:
                    XmppMasterDatabase().setUptime_machine(hostname, spresence, status=1,
                                                           updowntime=time.time() - lastevent[0]['time'], date=None)
            else:
                        ...
                        result1 = self.iqsendpulse(
                            presence['from'], {
                                "action": "information", "data": {
                                    "listinformation": ["force_reconf"], "param": {}}}, 5)
                except Exception:
                    pass
                try:

...
def Algorithm_Rule_Attribution_Agent_Relay_Server(objectxmpp,
    ...
    for x in ordre:
        ...
        # Load Balancer Rule : 6
            ...
            logger.debug("Analysis the 6th rule : select the relay on less " \
        ...
        try:
            ...
            if len(listars) == 0:
                listars = XmppMasterDatabase().getRelayServerofclusterFromjidars(result[2],
                                                                             moderelayserver="static",
                                                                             enablears = None)
                logger.warning("agent %s. ARS %s is found but it is stopped." % (data['information']['info']['hostname'], result[2]))

...
def action(xmppobject, action, sessionid, data, msg, ret, dataobj):
    ...
    try:
        ...
        logger.debug("=====================================================")
        logger.info(
            "Received inventory from %s in inventory substitute agent" %
            (msg['from']))
        try:
        ...
        try:
            ...
            logger.debug(
                "inject intentory to %s code wed %s" %
        ...
        XmppMasterDatabase().setlogxmpp(
            "Received inventory from machine %s" %
            msg['from'],
            "Inventory",
            "",
            0,
        ...
        if nbsize < 250:
            XmppMasterDatabase().setlogxmpp(
                '<span class="log_warn">Inventory XML size: %s byte</span>' %
                nbsize,
                "Inventory",
                "",
                0,
...
def XmppUpdateInventoried(jid, machine):
    ...
    try:
        ...
        logger.error(
            ...
            "make sure fusioninventory is running correctly" %
            machine)
    except Exception:
        logger.error(

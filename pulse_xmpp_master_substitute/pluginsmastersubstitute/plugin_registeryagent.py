...
def action(xmppobject, action, sessionid, data, msg, ret, dataobj):
    try:
        ...
        showinfobool = False
        listupt = [x.upper()
                   for x in xmppobject.registeryagent_showinfomachine]
        for x in listupt:
            if x in str(msg['from']).upper():
                logger.info(
                    "** Detailed information for machine %s" %
                    (str(
                        msg['from'])))
                showinfobool = True
                break
        ...
        if 'action' in data and data['action'] == 'infomachine':
            ...
            if machine:
                if 'regcomplet' in data and data['regcomplet'] is True or str(
                        jid.JID(data['from']).domain) != str(jid.JID(str(machine['jid'])).domain):
                    if showinfobool:
                        logger.info(
                            "Performing a complete re-registration of the machine %s" %
                            msg['from'])
                        logger.info(
                            "Deleting machine %s in machines table" %
                            msg['from'])
                    XmppMasterDatabase().delPresenceMachinebyjiduser(
                        msg['from'].user)
                    machine = {}
            ...
            if showinfobool:
                if len(machine) != 0:
                    logger.info(
                        "Machine %s already exists in base" %
                        msg['from'])
                else:
                    logger.info(
                        "Machine %s does not exist in base" %
                        msg['from'])
            if len(machine) != 0:
                ...
                try:
                    ...
                    if showinfobool:
                        logger.info(
                            "The mac addresses on the machine: %s " %
                            macadressstr)
                ...
                if machine['enabled'] == 1:
                    ...
                    if showinfobool:
                        logger.info(
                            ...
                            data['from'])
                    pluginfunction = [str("plugin_%s" % x)
                                      for x in xmppobject.pluginlistregistered]
                    if showinfobool:
                        logger.info(
                    ...
                    if showinfobool:
                        ...
                        logger.debug("=============")
                        logger.debug(
                            "Case 1 : The machine %s already exists : " % str(
                                msg['from']))
                    ...
                    if data['from'] != machine['jid'] or\
                        ...
                        XmppMasterDatabase().updateMachinejidGuacamoleGroupdeploy(
                            machine['jid'], machine['urlguacamole'], machine['groupdeploy'], machine['id'])
                    ...
                    if xmppobject.check_uuidinventory and \
                        ...
                        if machine['uuid_inventorymachine'] is not None:
                            if showinfobool:
                                logger.info(
                                    "Coherence True for hostname %s" % hostname)
                    if 'uuid_inventorymachine' not in machine or \
                        ...
                        if data['agenttype'] != "relayserver":
                            ...
                            btestfindcomputer, idglpimachine = test_consolidation_inventory(
                                xmppobject, sessionid, data, showinfobool, msg, machine['id'])
                            if showinfobool:
                        ...
                        if showinfobool:
                            logger.info(
                                "Machine %s already exists" %
                                data['from'])
                            logger.info(
                                "Verifying existence of jid %s" %
                                msg['from'])
            ...
            if showinfobool:
                logger.info(
                    "** Machine %s reports offline in machines table" %
                    data['from'])
                logger.info(
            ...
            if not is_masterpublickey:
                ...
                if showinfobool:
                    logger.info(
                        "** Machine %s key master public missing" %
                        data['from'])
                    logger.info(
                        "** call install key public master on Machine %s" %
                        data['from'])
                datasend = {"action": "installkeymaster",
            ...
            if showinfobool:
                ...
                logger.info("=============")
                logger.info(
                    "Case 2 : The machine %s does not exist in database" % str(
                        msg['from']))
                logger.info(
                    "Creating it and updating it's uuid_inventorymachine")
                logger.info("=============")
                logger.info("=============")
                logger.info(
                    "Adding or updating machine presence into machines table")
            for interface in data['information']["listipinfo"]:
            ...
            if idmachine != -1:
                ...
                if showinfobool:
                    logger.info(
                        "Machine %s added to machines table" %
                        idmachine)
                ...
                if useradd != -1:
                    ...
                    logger.warning(
                        "** No user found for the machine %s" %
                        msg['from'])
                    XmppMasterDatabase().setlogxmpp(
                ...
                if data['agenttype'] != "relayserver":
                    ...
                    if btestfindcomputer:
                        ...
                        if 'countstart' in data and data['countstart'] == 1:
                            if showinfobool:
                                logger.info(
                                    "** Calling inventory on PXE machine")
                            callinventory(xmppobject, data['from'])
                            return
            ...
            XmppMasterDatabase().setlogxmpp(
                ...
                xmppobject.boundjid.bare)
            pluginfunction = [str("plugin_%s" % x)
                              for x in xmppobject.pluginlistunregistered]
            if showinfobool:
                logger.info(
                    "Calling plugin action for machine %s ." %
                    msg['from'])
            for function_plugin in pluginfunction:
                try:
                    if hasattr(xmppobject, function_plugin):
                    ...
                    XmppMasterDatabase().setlogxmpp("Error on machine %s : %s" %
                                                    (msg['from'], str(e)), "info", sessionid, -
                                                    1, msg['from'], '', '', 'Registration | Notify | Error | Alert', '', '', xmppobject.boundjid.bare)
    except Exception as e:
        logger.error("Error on machine %s : %s\n%s" %
                     (msg['from'], str(e), traceback.format_exc()))
        XmppMasterDatabase().setlogxmpp(
            "Error on machine %s : %s" % (msg['from'],
                                          str(e)),
            "info",
            sessionid,
            -1,
...
def test_mac_address_black_list(
        macaddress,
        table_reg_for_match,
        showinfobool=True):
    if showinfobool:
...
def getComputerByMac(mac, showinfobool=True):
    if showinfobool:
        logger.info(
            "Function getComputerByMac asking glpi for machine list for mac %s" %
            mac)
...
def callInstallConfGuacamole(
    ...
    if 'remoteservice' in data and len(data['remoteservice']) > 0:
        ...
        if showinfobool:
            logger.info(
                "Setting guacamole parameters in base for machine id %s" %
                data['uuid'])
...
def test_consolidation_inventory(
    ...
    if isinstance(
            setupuuid,
            dict) and 'count' in setupuuid and setupuuid['count'] != 0:
        if showinfobool:
        ...
        try:
            ...
            logger.error(
                "Interface missing on machine %s id %s" %
                (msg['from'], idmachine))
            ...
            logger.error("List of interfaces on machine %s: %s" % (msg['from'],
                                                                   macmachine))
            logger.error(
                "Registration incomplete for machine %s" %
                msg['from'])
            XmppMasterDatabase().setlogxmpp(
        ...
        for t in results:
            if showinfobool:
                logger.info(
                    "Finding the machine which has the specified mac address : %s" %
                    t)
            if showinfobool:
                logger.info(
                    "Finding uuid for mac address %s for machine %s" %
                    (t, msg['from']))
...
def get_packages_for_machine(machine, showinfobool=True):
    ...
    softwareonmachine = Glpi().getLastMachineInventoryPart(
        machine['uuid_inventorymachine'], 'Softwares', 0, -1, '', {
    ...
    if showinfobool:
        logger.info(
            "* Initializing kiosk on machine %s" %
            (machine['hostname']))
...
def __search_software_in_glpi(
    ...
    for soft_glpi in list_software_glpi:
        ...
        if patternname.match(str(soft_glpi[0])) or patternname.match(
                str(soft_glpi[1])):
            ...
            if LooseVersion(soft_glpi[2]) < LooseVersion(packageprofile[3]):
                ...
                logger.debug(
                    ...
                    (packageprofile[0], soft_glpi[2], LooseVersion(
                        packageprofile[3])))
            break
...
def adduserdatageolocalisation(xmppobject, data, msg, sessionid, showinfobool):
    try:
        ...
        if 'users' in data['information'] and len(
        ...
        try:
            ...
            logger.error("\n%s" % (traceback.format_exc()))
            logger.error(
                "** Impossible to register machine %s. User missing" %
                msg['from'])
            XmppMasterDatabase().setlogxmpp(
                "Machine %s not registered. No user found" % msg['from'],
                "info",
...
def read_conf_remote_registeryagent(xmppobject):
    ...
    if not os.path.isfile(pathfileconf):
        ...
        if Config.has_option("parameters", "pluginlistregistered"):
            pluginlistregistered = Config.get(
                'parameters', 'pluginlistregistered')
        else:
            pluginlistregistered = "loadpluginlistversion, loadpluginschedulerlistversion,"\
                " loadautoupdate, loadshowregistration"
        ...
        if Config.has_option("parameters", "pluginlistunregistered"):
            pluginlistunregistered = Config.get(
                'parameters', 'pluginlistunregistered')
        else:
            pluginlistunregistered = "loadpluginlistversion, loadpluginschedulerlistversion,"\
                "loadautoupdate, loadshowregistration"
        ...
        blacklisted_mac_addresseslist = list(
            set(blacklisted_mac_addresseslist))

        for regexpconf in blacklisted_mac_addresseslist:
            try:
                logger.info(
                    "BUILD REGEXP FOR BLACKLIST MAC ADDRESS -> %s" %
                    regexpconf)
                xmppobject.blacklisted_mac_addresses.append(
                    re.compile(regexpconf))
            except Exception:
                logger.error(
        ...
        if Config.has_section("parameters"):
            if Config.has_option("parameters", "showinfomachine"):
                paramshowinfomachine = Config.get('parameters',
                                                  'showinfomachine')
                xmppobject.registeryagent_showinfomachine = [
                    str(x.strip()) for x in paramshowinfomachine.split(",") if x.strip() != ""]
            else:
    ...
    logger.debug(
        "Plugin list registered is %s" %
        xmppobject.pluginlistregistered)
    logger.debug("Plugin list unregistered is %s" %
                 xmppobject.pluginlistunregistered)

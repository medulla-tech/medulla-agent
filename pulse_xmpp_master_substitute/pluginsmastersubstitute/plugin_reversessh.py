...
def action(xmppobject, action, sessionid, data, message, ret, dataobj):
    ...
    try:
        ...
        logger.debug(
            "data[0] not found while calling %s. "
            "The plugin is probably called from a quick action." % (plugin["NAME"])
        )
        pass
    ...
    try:
        ...
        if len(params) == 3:
            ...
            logger.error(
                "If called from a quick action, the following "
    ...
    logger.info(
        "Starting reversessh connection from machine %s.\n"
        " Tunnel server port: %s\n"
        " Tunnel client port: %s\n"
        " Reverse type: %s"
        % (jidmachine, tunnelserverport, tunnelclientport, reversetype)
    )

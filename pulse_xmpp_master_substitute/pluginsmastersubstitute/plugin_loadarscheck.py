...
def action(objectxmpp, action, sessionid, data, msg, ret):
    ...
    try:
        ...
        logger.error(
            "Plugin loadarscheck, we encountered the error %s" %
            str(e))
                ...
                if self.update_table:
                    ...
                    if self.monitoring_message_on_machine_no_presence:
                        ...
                        self.message_datas_to_monitoring_loadarscheck(
                            jidaction, "The Ars %s is down" %
                            jidaction, informationaction="ack")
        ...
        for ars in disabled_ars:
            ...
            if arsstatus['server']['presence'] == 1 and \
                ...
                self.xmpplog("The ARS %s is online" % ars['jid'],
                             ...
                             date=None,
                             fromuser=ars['jid'])
                XmppMasterDatabase().update_Presence_Relay(
                    ars['jid'], presence=1)
    except Exception as e:
        logger.error("We failed to check the ARS Status")
        logger.error(
            "The backtrace of this error is \n %s" %
...
def read_conf_loadarscheck(objectxmpp):
    ...
    if not os.path.isfile(pathfileconf):
        ...
        if ars_config.has_option("parameters",
                                 "monitoring_message_on_machine_no_presence"):
            objectxmpp.monitoring_message_on_machine_no_presence = ars_config.getboolean(
        ...
        if ars_config.has_option("parameters", "monitor_agent"):
            objectxmpp.monitor_agent = ars_config.get(
                'parameters', 'monitor_agent')
        else:
            # default values parameters
            objectxmpp.monitor_agent = "master_mon@pulse"
    ...
    logger.debug(
        "parameter loadarscheck : update_table = %s" %
        objectxmpp.update_table)
    ...
    if objectxmpp.update_table:
        ...
        logger.debug(
            ...
            objectxmpp.check_ars_scan_interval)
        logger.debug(
            "parameter loadarscheck : monitor_agent = %s" %
            objectxmpp.monitor_agent)
    logger.debug("lock ressource_scan_available = %s" %
                 objectxmpp.ressource_scan_available)

...
DEBUGPULSEPLUGIN = 25
plugin = {"VERSION": "1.0", "NAME": "loadshowregistration", "TYPE": "substitute"}  # fmt: skip
...
def plugin_loadshowregistration(self, msg, data):
    if (
        "all" in self.showinfomachine
        or "ALL" in self.showinfomachine
        or data["machine"].split(".")[0] in self.showinfomachine
    ):

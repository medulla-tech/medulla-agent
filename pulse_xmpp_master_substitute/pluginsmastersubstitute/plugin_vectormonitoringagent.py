...
def process_system(functionname,
    ...
    if 'alarms' in data:
        if isinstance(data['alarms'], basestring):
            ...
            alarm_msg = data['alarms']
        del data['alarms']
    logger.debug("(system) call setMonitoring_device_reg hostname %s\n"\
                                        " id_machine %s \n"\
                                        " platform %s \n"\
                                        " agenttype %s \n"\
                                        " statusmsg %s \n"\
    ...
    a= XmppMasterDatabase().setMonitoring_device_reg(hostname,
                                                    id_machine,
                                                    platform,
                                                    agenttype,
                                                    statusmsg,
...
def process_nfcreader(functionname,
    ...
    if 'message' in data:
        if isinstance(data['message'], basestring):
            ...
            alarm_msg = data['message']
        del data['message']
    logger.debug("(nfcreader) call setMonitoring_device_reg hostname %s\n"\
                                        " id_machine %s \n"\
                                        " platform %s \n"\
                                        " agenttype %s \n"\
                                        " statusmsg %s \n"\
...
def process_generic(functionname,
    ...
    if 'message' in data:
        if isinstance(data['message'], basestring):
            ...
            alarm_msg = data['message']
        del data['message']
    logger.debug("(generic) call setMonitoring_device_reg hostname %s\n"\
                                        " id_machine %s \n"\
                                        " platform %s \n"\
                                        " agenttype %s \n"\
                                        " statusmsg %s \n"\
                    ...
                    if devicename.lower() in xmppobject.typelistMonitoring_device:
                        if devicename in element:
                            if "subject" in element[devicename]:
                                statusmsg['mon_subject'] = element[devicename]['subject']
                ...
                for devicename in element:
                    ...
                    if devicename.lower() in xmppobject.typelistMonitoring_device:
                        if devicename in element:
                            ...
                            if "param0" in element[devicename]:
                                statusmsg['mon_param0'] = element[devicename]['param0']
                        result = callFunction(devicename,
                                              xmppobject,
                                              str(message['from']),
                                              sessionid,
                                              element[devicename],
                                              ...
                                              id_mom_machine)
                        if result  == -1:
                            logger.warning("[%s] verify message alert from %s"%(sessionid,message['from']))
                            logger.warning("[%s] binding peut etre pas resolue"%sessionid)

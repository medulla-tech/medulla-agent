...
plugin = {"VERSION": "1.03", "NAME": "registeryagent", "TYPE": "substitute"}
...
def action(xmppobject, action, sessionid, data, msg, ret, dataobj):
    try:
        ...
        ### xmppobject.registeryagent_showinfomachine=['P516901N6900034']
        logger.debug("includ list jid %s"%(xmppobject.registeryagent_showinfomachine))
        logger.debug("FROM %s"%(msg['from']))
        showinfobool = False
        listupt = [ x.upper() for x in xmppobject.registeryagent_showinfomachine]
        for x in listupt:
            if x in str(msg['from']).upper():
                logger.debug("Match beetween %s and %s"%(x , str(msg['from'])))
                showinfobool = True
                break
            else:
                showinfobool = False
        ...
        if 'action' in data and data['action'] == 'infomachine':
            ...
            if len(machine) != 0:
                if showinfobool:
                    logger.warning("***** machine %s : PRESENTE IN TABLE"%data['from'])
            else:
                if showinfobool:
                    logger.warning("***** machine %s :NON PRESENTE IN TABLE"%data['from'])
            if len(machine) != 0:
                        ...
                        if data['agenttype'] != "relayserver":
                            ...
                            for testinventaireremonte in range(20):
                                ...
                                for macaddress in results:
                                    ...
                                    if macaddress in xmppobject.blacklisted_mac_addresses:
                                        if showinfobool:
            ...
            try:
                ...
                if 'users' in data['information'] and len(data['information']['users']) > 0:
                    if showinfobool:
                        logger.info("** addition user %s in base" %
            ...
            if data['agenttype'] == "relayserver":
                ...
                for ARScluster in listrelayserver:
                    xmppobject.send_message(mto=ARScluster,
                                        ...
                                        mtype='chat')
            if showinfobool:
                logger.info("** Add machine in database")
            # Add machine
            ippublic = None
            if "ippublic" in data:
            ...
            if showinfobool:
                ...
                logger.info("=============")
                logger.info("=============")
                logger.info("Adding new machine presence into machines table")
            ...
            if idmachine != -1:
                ...
                for i in data['information']["listipinfo"]:
                    ...
                    if showinfobool:
                        ...
                        logger.info("Add network card %s to the machine #%s"%(i['macaddress'],
                                                                               idmachine))
                ...
                if data['agenttype'] != "relayserver":
                    ...
                    #logger.info(
                        ...
                        #     "call Guacamole config\nor add inventory")
                    if showinfobool:
                        logger.info("List the mac addresses for the machine #%s"%idmachine)
                    ...
                    for testinventaireremonte in range(20):
                        for t in results:
                            ...
                            if computer != None:
                                computerid = str(computer.id)
                                if showinfobool:
                                    logger.info("Id found : %s"%computer.id)
                                jidrs = str(jid.JID(data['deployment']).user)
                                jidm = jid.JID(data['from']).domain
                                jidrs = "%s@%s" % (jidrs, jidm)
                                uuid = 'UUID' + str(computer.id)
                        ...
                        if btestfindcomputer == False:
                            ...
                            if showinfobool:
                                logger.info("callInstallConfGuacamole on 1 %s for %s"%(jidrs,
                                                                                        data['information']['info']['hostname']))
                            callInstallConfGuacamole(xmppobject,
                                        jidrs,
                                        {  'hostname': data['information']['info']['hostname'],
                ...
                logger.error("** Database registration error for machine %s"%msg['from'])
                return

            pluginfunction=[str("plugin_%s"%x) for x in xmppobject.pluginlistunregistered]
            if showinfobool:
                logger.info("call plugin for machine %s not present.."%msg['from'])
            for function_plugin in pluginfunction:
                try:
                    if hasattr(xmppobject, function_plugin):
                        if showinfobool:
...
def getComputerByMac( mac, showinfobool=True):
    if showinfobool:
        logger.info("Asking to glpi the machine list for specified mac ...")
    ...
    if type(ret) == list:
        if len(ret) != 0:
            ...
            return None
    if showinfobool:
        logger.info("Glpi returned : %s"%ret)
    return ret

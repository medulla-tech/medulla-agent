...
def Action_update(self):
    ...
    try:
        ...
        if pidlist:
            ...
            pid_finish = [
                int(x["id"]) for x in pidlist if x["pid_run"] not in list_pid_bash
            ]
                ...
                if rr["code"] == 0:
                    for ligneresult in rr["result"]:
                        if ligneresult.startswith("pid : "):
                            ...
                            self.msg_debug_local(
                                "pid programme launcher : %s" % pid_run
                            )
                            # on met a jour le pid
                            XmppMasterDatabase().update_pid_all_Up_action_update_packages(
                                int(t["id"]), pid_run
                            )
        self.msg_debug_local("===================Action_update=====================")
    except Exception as e:

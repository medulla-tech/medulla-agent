...
def action(objectxmpp, action, sessionid, data, msg, dataerreur):
    ...
    if compteurcallplugin == 0:
        ...
        objectxmpp.loadreconf = types.MethodType(loadreconf, objectxmpp)
...
def loadreconf(self, objectxmpp):
    ...
    while time.time() < end:
        ...
        if len(list_need_reconf) > 0:
            resultacquite = XmppMasterDatabase().call_acknowledged_reconficuration(
                list_need_reconf
            )
            # liste des concurent
        ...
        while (
            len(objectxmpp.listconcurentreconf) < objectxmpp.nbconcurrentreconf
            and len(result) > 0
            and time.time() < end
        ):
            ...
            self.send_message(
                mto=eltmachine[1], mbody=json.dumps(datasend), mtype="chat"
            )
            logger.debug(
                "SEND RECONFIGURATION %s (%s)" % (eltmachine[1], eltmachine[0])
            )
            list_updatenopresence.append(eltmachine[0])
        if len(list_updatenopresence) != 0:
            XmppMasterDatabase().call_set_list_machine(
                listmachine=list_updatenopresence
            )
        time.sleep(0.2)
...
def read_conf_loadreconf(objectxmpp):
    ...
    if not os.path.isfile(pathfileconf):
        logger.warning(
            ...
            "timeout_reconf = 500" % (plugin["NAME"], pathfileconf)
        )
        ...
        if Config.has_option("parameters", "generate_reconf_interval"):
        ...
        if Config.has_option("parameters", "concurrentreconf"):
            objectxmpp.nbconcurrentreconf = Config.getint(
                "parameters", "concurrentreconf"
            )
        else:
            objectxmpp.nbconcurrentreconf = 240

        if Config.has_option("parameters", "timeout_reconf"):
            objectxmpp.timeout_reconf = Config.getint("parameters", "timeout_reconf")
        else:
            objectxmpp.timeout_reconf = 500

...
def action(xmppobject, action, sessionid, data, message, ret, dataobj):
    ...
    uuid_machine = XmppMasterDatabase().getUuidSerialMachineFromJid(message["from"])[
        "uuid_serial"
    ]  # retrieve id from jid

    for antivirus in data:  # add each antivirus
        u_status = (
            1 if antivirus["Update Status"] != "[Disabled]" else 0
        )  # update status
        rt_status = (
            1 if antivirus["Real-time Protection Status"] != "[Disabled]" else 0
        )  # check if realtime protection is enabled
        active_status = (
            1 if u_status and rt_status else 0
        )  # check if it's the active antivirus in the list
        antivirus_name = str(antivirus["Name"])
        firewall_status = antivirus["Firewall"]
        # Handle Last Scan field
        last_scan = antivirus["Last Scan"]

        XmppMasterDatabase().updateAntivirusFirewallStatus(
            uuid_machine,
            antivirus_name,
            u_status,
            rt_status,

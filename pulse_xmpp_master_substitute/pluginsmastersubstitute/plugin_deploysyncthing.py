...
def action(xmppobject, action, sessionid, data, message, dataobj):
    ...
    if "subaction" in data:
        ...
        if "counttransfertterminate" in data["subaction"]:
            ...
            XmppMasterDatabase().incr_count_transfert_terminate(
                data["iddeploybase"])
            ...
            # logger.debug("=====================================================")
            listclusterobjet = XmppMasterDatabase(
            ).getCluster_deploy_syncthing(data['iddeploy'])
            ...
            for index, value, in enumerate(clustersdata['listarscluster']):
                ...
                if [x for x in keycheck if x in infoars] == keycheck:
                    adressipserver = "tcp://%s:%s" % (
                        infoars['ipserver'], infoars['syncthing_port'])
                    adressconnection = "tcp://%s:%s" % (
                        infoars['ipconnection'], infoars['syncthing_port'])
                    clu['arsip'][val] = [
                        str(adressipserver),
            ...
            deploy_syncthing_information['agentdeploy'] = str(
            ...
            datasend = {
                ...
                "sessionid": name_randomplus(
                    30,
                    "syncthingclusterinit"),
                "ret": 0,
                "base64": False,
                "data": {
                    "subaction": "syncthingdeploycluster"}}

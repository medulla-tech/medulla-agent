...
class MUCBot(slixmpp.ClientXMPP):
    ...
    def Clean_old_queue(self, nbsecond):
        ...
        """
        queue_files = [queue_file for queue_file in os.listdir("/dev/mqueue") if queue_file != "mysend" and os.path.isfile(os.path.join("/dev/mqueue", queue_file))]
        for queue_file in queue_files:
            ...
            if (time.time() - os.path.getmtime(path_queue) > nbsecond):
                try:
                    ...
                    logger.debug("An error occured while deleting the file %s from the queue" queue_file)
    ...
    def clean_my_mpqueue(self):
        ...
        """
        mpqueue_files = [mpqueue_file for mpqueue_file in listdir("/dev/mqueue")
            ...
            if mpqueue_file != 'mysend':
                if mpqueue_file.startswith("/" + self.boundjid.user):
                    try:
                        ...
                        logger.error("An error occured while deleting the file %s" % mpqueue_file)

...
class manage_grafana:
    ...
    def grafanaGetDashboardForHostname(self):
        ...
        try:
            ...
            if r.status_code != requests.codes.ok:
                logger.error("Error getting dashboard for hostname %s"
                             % self.hostname)
            ...
            logger.debug("JSON dashboard response: %s"
                         % json.dumps(r.json(), indent=4))
            return r.json()
        except requests.RequestException as e:
            logger.error("Error getting dashboard for hostname %s: %s"
                         % (self.hostname, str(e)))
            return None
    ...
    def grafanaWriteDashboardForHostname(self, dashb_json):
        ...
        try:
            ...
            if r.status_code != requests.codes.ok:
                logger.error("Error writing dashboard for hostname %s"
                             % self.hostname)
                logger.error("Received status code %s" % r.status_code)
                logger.error("Complete response: %s" % r.text)
                return None
            ...
            logger.error("Error writing dashboard for hostname %s: %s"
                         % (self.hostname, str(e)))
            return None
    ...
    def grafanaCreateDashboard(self):
        ...
        if dashb_id:
            logger.info("Dashboard %s created for hostname %s"
                        % (dashb_id, self.hostname))
            return dashb_id
        # Error writing dashboard
        logger.error("Error creating dashboard for hostname %s"
                     % self.hostname)
        return None
    ...
    def grafanaDeleteDashboard(self):
        ...
        try:
            ...
            if r.status_code != requests.codes.ok:
                logger.error("Error deleting dashboard for hostname %s"
                             % self.hostname)
                logger.error("Received status code %s" % r.status_code)
                logger.error("Complete response: %s" % r.text)
                return None
            ...
            logger.error("Error deleting dashboard for hostname %s: %s"
                         % (self.hostname, str(e)))
            return None

    def grafanaDeletePanel(self, panel_title):
        # Get dashboard for hostname and delete the panel
        logger.info("Deleting panel %s for hostname %s" % (panel_title,
                                                           self.hostname))
        db_json = self.grafanaGetDashboardForHostname()
        ...
        if panel_index:
            ...
            dashb_id = self.grafanaWriteDashboardForHostname(db_json)
            if dashb_id:
                logger.info("Panel %s deleted from dashboard %s "
                            "for hostname %s" % (panel_title,
                                                 dashb_id,
                                                 self.hostname))
                return True
            # Error writing dashboard
            logger.error("Error deleting panel %s from dashboard "
                         "for hostname %s" % (panel_title, self.hostname))
            return False
        # No panel found
        logger.error("Error deleting panel %s from dashboard "
                     "for hostname %s: Pannel not found"
                     % (panel_title, self.hostname))
        return False
    ...
    def grafanaGetPanelGraph(self, panel_title, from_timestamp, to_timestamp):
        ...
        logger.debug("Getting graph url for panel %s for hostname %s"
                     % (panel_title, self.hostname))
        panel_id = self.grafanaGetPanelIdFromTitle(panel_title)
        if panel_id:

...
class MscDatabase(DatabaseHelper):
    ...
    def initTables(self):
        ...
        try:
            # commands
            self.commands = Table(
                "commands", self.metadata, Column(
                    'dispatched', String(32), default='YES'), Column(
                    'fk_bundle', Integer, ForeignKey('bundle.id')), autoload=True)
            # commands_history
            ...
            self.pull_targets = Table(
                "pull_targets", self.metadata, autoload=True)
            ...
            self.commands_on_host = Table(
                "commands_on_host", self.metadata, Column(
                    'fk_commands', Integer, ForeignKey('commands.id')), Column(
                    'fk_target', Integer, ForeignKey('target.id')), autoload=True)
    ...
    def getIdCommandOnHost(self, ctx, id):
        ...
        query = session.query(CommandsOnHost).select_from(
            self.commands_on_host.join(
                self.commands)).filter(
            self.commands.c.id == id)
    ...
    def deleteBundle(self, bundle_id):
        ...
        try:
            ...
            self.logger.error(
                "Delete of bundle (id=%s) failed: %s" %
                (bundle_id, str(exc)))
            session.rollback()
            session.close()
            return False
    ...
    def xmpp_create_CommandsOnHostPhasedeploy(
            self, fk_commands, name, state="ready"):
        session = create_session()
    ...
    def updategroup(self, group):
        session = create_session()
        join = self.commands_on_host.join(
            self.commands).join(
            self.target).join(
            self.commands_on_host_phase)
        q = session.query(
            CommandsOnHost,
            Commands,
            Target,
        ...
        for x in q:
            ...
            session.query(CommandsOnHostPhase).filter(
                CommandsOnHostPhase.fk_commands_on_host == x.CommandsOnHost.id). update(
                {
                    CommandsOnHostPhase.state: "done"})
            session.flush()
        session.flush()
        session.close()
        return result
    ...
    def xmppstage_statecurrent_xmpp(self):
        ...
        session.query(CommandsOnHost).filter(
            and_(
                ...
                CommandsOnHost.end_date < datenow)). update(
            {
                CommandsOnHost.current_state: "over_timed",
                CommandsOnHost.stage: "ended"})
        session.flush()
    ...
    def deployxmpp(self, session, limitnbr=100):
        ...
        try:
            ...
            session.execute(
                "LOCK TABLES commands_on_host WRITE, commands READ , target READ, phase WRITE;")
            ...
            for msc_machine_to_deploy in selectedMachines:
                ...
                self.logger.debug(
                    "The computer %s (id: %s) is"
                    "available to deployment the package %s" %
                    ...
                    updatemachine.append(
                        {
                            ...
                            'mac': str(
                                msc_machine_to_deploy.target_target_macaddr),
                            'count': 0,
                            'cycle': 0,
                            'login': str(
                                msc_machine_to_deploy.commands_creator),
    ...
    def get_deploy_inprogress_by_team_member(
        ...
        query = session.query(
            ...
            Target.target_uuid,
            Target.id_group,
            Target.target_macaddr) .join(
            CommandsOnHost,
            Commands.id == CommandsOnHost.fk_commands) .join(
                Target,
                Target.id == CommandsOnHost.fk_target) .join(
                    CommandsOnHostPhase,
                    CommandsOnHostPhase.fk_commands_on_host == CommandsOnHost.id) .filter(
                        CommandsOnHostPhase.name == 'upload') .filter(
                            CommandsOnHostPhase.state == 'ready') .filter(
                                Commands.end_date > datereduced)
            ...
            query = query.filter(
                or_(
                    Commands.title.like(
                        "%%%s%%" %
                        filt),
                    Commands.creator.like(
                        "%%%s%%" %
                        filt),
                    Commands.package_id.like(
                        "%%%s%%" %
                        filt),
                    Commands.start_date.like(
                        "%%%s%%" %
                        filt),
                    Commands.end_date.like(
                        "%%%s%%" %
                        filt),
                    CommandsOnHost.id.like(
                        "%%%s%%" %
                        filt),
                    Target.target_name.like(
                        "%%%s%%" %
                        filt),
                    Target.target_uuid.like(
                        "%%%s%%" %
                        filt),
                    Target.id_group.like(
                        "%%%s%%" %
                        filt),
                    Target.target_macaddr.like(
                        "%%%s%%" %
                        filt)))
    ...
    def deleteCommand(self, cmd_id):
        ...
        try:
            ...
            self.logger.error(
                "Delete of command (id=%s) failed: %s" %
                (cmd_id, str(exc)))
            session.rollback()
            session.close()
            return False
    ...
    def deleteCommandOnHost(self, coh_id):
        ...
        try:
            ...
            self.logger.error(
                "Delete of command on host(id=%s) failed: %s" %
                (coh_id, str(exc)))
            session.rollback()
            session.close()
            return False
    ...
    def _deleteCommands(self, session, cmds):
        ...
        for cmd in cmds.all():
            ...
            if ok:
                ...
                session.flush()
                self.logger.info(
                    "Command (id=%s) successfully deleted" %
                    (cmd.id))

            else:
                self.logger.warn(
                    "Unable to delete commands on host of command (id=%s)" %
                    cmd.id)
                return False
    ...
    def _deleteCommandsOnHost(self, session, cohs):
        ...
        for coh in cohs.all():
            ...
            hists = hists.filter(
                self.commands_history.c.fk_commands_on_host == coh.id)
        ...
        for coh in cohs:
            ...
            for name in wf_list:
                ...
                if name == "upload" and len(files) == 0:
                    continue
                if name == "execute" and (
                        start_script == "disable" or is_quick_action) and do_windows_update == "disable":
                    continue
    ...
    def getCommandsonhostsAndSchedulersOnBundle(self, fk_bundle):
        ...
        c_ids = select([self.commands.c.id],
                       self.commands.c.fk_bundle == fk_bundle).execute()
        c_ids = map(lambda x: x[0], c_ids)
        result = select([self.commands_on_host.c.id,
                         self.commands_on_host.c.scheduler],
                        self.commands_on_host.c.fk_commands.in_(c_ids)).execute()
        schedulers = {}
        for row in result:
            coh, scheduler = row
    ...
    def getCommandsonhostsAndSchedulers(self, c_id):
        ...
        result = select([self.commands_on_host.c.id,
                         self.commands_on_host.c.scheduler],
                        self.commands_on_host.c.fk_commands == c_id).execute()
        schedulers = {}
        for row in result:
            coh, scheduler = row
            if scheduler in schedulers:
    ...
    def getAllCommandsonhostCurrentstate(self, ctx):
        ...
        ret = ret.add_column(
            self.commands.c.max_connection_attempt).filter(
            self.commands_on_host.c.current_state != ''). group_by(
            self.commands_on_host.c.current_state). group_by(
                self.commands_on_host.c.attempts_left). group_by(
                    self.commands.c.max_connection_attempt). order_by(
                        asc(
                            self.commands_on_host.c.next_launch_date))
    ...
    def countAllCommandsonhostByCurrentstate(
            self, ctx, current_state, filt=''):
        # TODO use ComputerLocationManager().doesUserHaveAccessToMachine
        session = create_session()
        ret = self.__queryAllCommandsonhostBy(session, ctx)
        if current_state == 'rescheduled':  # patch to have rescheduled as a "state" ... must be emulated
            ret = ret.filter(
                and_(
                    self.commands.c.max_connection_attempt != self.commands_on_host.c.attempts_left,
            ...
            ret = ret.filter(
                self.commands_on_host.c.current_state == current_state)
        # the join in itself is useless here, but we want to have exactly
        # the same result as in getAllCommandsonhostByCurrentstate
        if filt != '':
            ret = ret.filter(or_(self.commands_on_host.c.host.like(
                '%' + filt + '%'), self.commands.c.title.like('%' + filt + '%')))
        c = ret.count()
        session.close()
        return c
    ...
    def countAllCommandsOnHostBundle(
            self, ctx, uuid, fk_bundle, filt, history):
        # TODO use ComputerLocationManager().doesUserHaveAccessToMachine
        session = create_session()
        ret = session.query(CommandsOnHost).select_from(
            self.commands_on_host.join(
                self.commands).join(
                self.target)).filter(
            self.target.c.target_uuid == uuid).filter(
                    self.commands.c.creator == ctx.userid).filter(
                        self.commands.c.fk_bundle == fk_bundle)
        ...
        if ComputerLocationManager().doesUserHaveAccessToMachine(ctx, uuid):
            ...
            ret = session.query(CommandsOnHost).select_from(
                self.commands_on_host.join(
                    self.commands).join(
                    self.target)).filter(
                self.target.c.target_uuid == uuid)
            if filt != '':
                ret = ret.filter(self.commands.c.title.like('%' + filt + '%'))
            c = ret.count()
            session.close()
            return c
        self.logger.warn(
            "User %s does not have good permissions to access '%s'" %
            (ctx.userid, uuid))
        return False
    ...
    def getAllCommandsOnHost(self, ctx, uuid, min, max, filt):
        if ComputerLocationManager().doesUserHaveAccessToMachine(ctx, uuid):
            session = create_session()
            query = session.query(Commands).add_column(
                self.commands_on_host.c.id).add_column(
            ...
            if filt != '':
                query = query.filter(
                    self.commands.c.title.like(
                        '%' + filt + '%'))
            query = query.order_by(
                asc(self.commands_on_host.c.next_launch_date))
    ...
    def getAllCommandsConsult(self, ctx, min, max, filt, expired=True):
        ...
        if filt:
            filters = and_(
                filters, or_(
                    self.commands.c.title.like(
                        '%%%s%%' %
                        (filt)), self.commands.c.creator.like(
                        '%%%s%%' %
                        (filt)), self.commands.c.start_date.like(
                        '%%%s%%' %
                        (filt)), self.bundle.c.title.like(
                        '%%%s%%' %
                        (filt)), self.target.c.target_name.like(
                            '%%%s%%' %
                        (filt))))
        ...
        query = query.select_from(
            self.commands.join(
                self.commands_on_host,
                self.commands_on_host.c.fk_commands == self.commands.c.id) .join(
                self.target,
                self.commands_on_host.c.fk_target == self.target.c.id) .outerjoin(
                self.bundle,
        ...
        query = query.add_column(
            self.commands.c.fk_bundle).add_column(
            self.commands_on_host.c.host).add_column(
            self.commands_on_host.c.id)
        query = query.add_column(
            self.target.c.id_group).add_column(
            self.bundle.c.title).add_column(
        ...
        query = query.select_from(
            self.commands.join(
                ...
                self.commands_on_host.c.fk_target == self.target.c.id) .outerjoin(
                self.bundle,
                self.commands.c.fk_bundle == self.bundle.c.id)) .outerjoin(
                    self.pull_targets,
        ...
        # Grouping bundle commands by fk_bundle only if fk_bundle is not null
        # So we generate random md5 hash for command that have null fk_bundle
        query = query.group_by(func.ifnull(self.commands.c.fk_bundle,
                                           func.md5(self.commands.c.id)))
    ...
    def __displayLogsQuery2(self, ctx, params, session, count=False):
        ...
        if count:
            ...
            query = query.add_column(
                self.commands_on_host.c.id).add_column(
                self.commands_on_host.c.current_state).add_column(
                PullTargets.target_uuid)
        ...
        if params['filt'] is not None:  # Filter on a commande names
            filter.append(
                self.commands.c.title.like(
                    '%s%s%s' %
                    ('%', params['filt'], '%')) | self.target.c.target_name.like(
                    '%s%s%s' %
                    ('%', params['filt'], '%')))
    ...
    def __displayLogReturn(self, ctx, list):
        ...
        for element in list:
            if element[1] in cohs:
                if len(element) == 4:
                    ret.append((element[0].toH(),
                                element[1],
                                element[2],
                                cohs[element[1]].toH(),
                                element[3]))
                else:
                    ret.append((element[0].toH(), element[1],
                               element[2], cohs[element[1]].toH()))
            else:
    ...
    def displayLogs(self, ctx, params=None):  # TODO USE ctx
        ...
        try:
            params['order_by'] = getattr(
                self.commands_on_host.c, params['order_by'])
        ...
        if params['gid'] or params['uuid']:  # we want informations about one group / host
            if params['cmd_id']:  # we want informations about one command on one group / host
                ...
                ret = self.__displayLogsQuery2(ctx, params, session).offset(
                    int(params['min'])).limit(int(params['max']) - int(params['min'])).all()
                ...
                # Using min/max, we get a range of commands, but we always want
                # the total count of commands.
                ret = self.__displayLogsQuery2(ctx, params, session).order_by(self.commands.c.order_in_bundle).offset(
                    int(params['min'])).limit(int(params['max']) - int(params['min'])).all()
                size = self.__displayLogsQuery2(ctx, params, session, True)
                session.close()
                return size, self.__displayLogReturn(ctx, ret)
                ...
                # id
                ret = self.__displayLogsQuery(ctx, params, session).order_by(
                    asc(params['order_by'])).all()
                cmds = []
                for c in ret:
                ...
                if params['uuid']:
                    ...
                    query = query.filter(
                        self.target.c.target_uuid == params['uuid'])
            ...
            if params['cmd_id']:  # we want all informations about one command
                ...
                ret = self.__displayLogsQuery2(
                    ctx, params, session).order_by(
                    self.commands.c.order_in_bundle).all()
                # FIXME: using distinct, size will always return 1 ...
                size = self.__displayLogsQuery2(ctx, params, session, True)
                session.close()
                return size, self.__displayLogReturn(ctx, ret)
    ...
    def getCommandsOnHosts(self, ctx, coh_ids):
        session = create_session()
        cohs = session.query(CommandsOnHost).add_column(
            self.commands_on_host.c.id).filter(
            self.commands_on_host.c.id.in_(coh_ids)).all()
        session.close()
        ...
        if ComputerLocationManager().doesUserHaveAccessToMachines(
            ...
            for e in cohs:
                # Loading coh phases
                e[0].phases = session.query(CommandsOnHostPhase).filter_by(
                    fk_commands_on_host=e[1]).all()
    ...
    def getCommandsOnHost(self, ctx, coh_id):
        ...
        if coh is None:
            self.logger.warn(
                ...
                (ctx.userid, coh_id))
            return False
        coh.phases = session.query(CommandsOnHostPhase).filter_by(
            fk_commands_on_host=coh_id).all()
    ...
    def getTargetForCoh(self, ctx, coh_id):  # FIXME should we use the ctx
        # TODO use ComputerLocationManager().doesUserHaveAccessToMachine
        session = create_session()
        target = session.query(Target).select_from(
            self.target.join(
                self.commands_on_host)).filter(
            self.commands_on_host.c.id == coh_id).first()
        session.close()
        return target
    ...
    def getCommandsHistory(self, ctx, coh_id):  # FIXME should we use the ctx
        # TODO use ComputerLocationManager().doesUserHaveAccessToMachine
        session = create_session()
        ret = session.query(CommandsHistory).filter(
            self.commands_history.c.fk_commands_on_host == coh_id).all()
        session.close()
    ...
    def getCommands(self, session, ctx, cmd_id):
        ...
        if ComputerLocationManager().doesUserHaveAccessToMachines(ctx, a_targets):
            ...
            command, coh = session.query(Commands).filter_by(
                id=cmd_id) .add_entity(CommandsOnHost) .outerjoin(
                (CommandsOnHost, Commands.id == CommandsOnHost.fk_commands)).first()
            if coh is not None:
                phases = session.query(CommandsOnHostPhase).filter_by(
                    fk_commands_on_host=coh.id).all()
    ...
    def getCommandsByGroup1(self, gid):
        session = create_session()
        ret = session.query(Commands).select_from(
            self.commands.join(
                self.commands_on_host).join(
                self.target)).filter(
            self.target.c.id_group == gid)
    ...
    def isPullTarget(self, session, uuid):
        try:
            session.query(PullTargets).filter(
                PullTargets.target_uuid == uuid).one()
            return True
            ...
            ret = connection.execute(
                select(
                    ...
                    and_(
                        ...
                        self.target.c.id == self.commands_on_host.c.fk_target))).fetchall()
        else:
            session = create_session()
            ret = session.query(Target).select_from(
                self.target.join(
                    self.commands_on_host)).filter(
                self.commands_on_host.c.fk_commands == cmd_id).all()
            session.close()
        return ret

    def getCommandOnHostCurrentState(self, ctx, cmd_id):
        session = create_session()
        ret = session.query(Commands).add_column(
            self.commands_on_host.c.current_state).select_from(
            self.commands.join(
                self.commands_on_host)).filter(
                self.commands.c.id == cmd_id).first()
        session.close()
        return ret[1]
    ...
    def getCommandOnHostInCommands(self, ctx, cmd_id):
        session = create_session()
        ret = session.query(CommandsOnHost).filter(
            self.commands_on_host.c.fk_commands == cmd_id).all()
        session.close()
    ...
    def getarraystatbycmd(self, ctx, arraycmd_id):
        ...
        session = create_session()
        ret = session.query(
            CommandsOnHost.fk_commands.label("idcmd"), func.count(
                self.commands_on_host.c.current_state).label("nb")). filter(
            and_(
                self.commands_on_host.c.fk_commands.in_(arraycmd_id))). group_by(
                    self.commands_on_host.c.fk_commands)
        ret.all()
        for x in ret:
    ...
    def getFirstCommandsOncmd_id(self, ctx, cmd_id):
        session = create_session()
        ret = session.query(CommandsOnHost).filter(
            self.commands_on_host.c.fk_commands == cmd_id).first()
        session.close()
        return ret
    ...
    def _getcommanddatadate(self, CommandsOnHostdata):
        start_dateunixtime = time.mktime(
            CommandsOnHostdata.start_date.timetuple())
    ...
    def getLastCommandsOncmd_id(self, ctx, cmd_id):
        session = create_session()
        ret = session.query(CommandsOnHost). filter(
            self.commands_on_host.c.fk_commands == cmd_id).order_by(
            desc(
                self.commands_on_host.c.id)).first()
        session.close()
    ...
    def getLastCommandsOncmd_id_start_end(self, ctx, cmd_id):
        ...
        ret = session.query(
            ...
            CommandsOnHost.next_launch_date). filter(
            self.commands_on_host.c.fk_commands == cmd_id).order_by(
            desc(
                self.commands_on_host.c.id)).first()
        session.close()
    ...
    def getCommandOnGroupByState(self, ctx, cmd_id, state, min=0, max=-1):
        session = create_session()
        query = session.query(CommandsOnHost).add_column(
            self.target.c.target_uuid).select_from(
            self.commands_on_host.join(
                self.commands).join(
                self.target)).filter(
                    self.commands.c.id == cmd_id).order_by(
                        self.commands_on_host.c.host)
        ret = self.__filterOnStatus(ctx, query, state)
        session.close()
        if max != -1:
            ret = ret[min:max]
        return [{'coh_id': coh.id,
                 'uuid': coh.target_uuid,
                 'host': coh.host,
                 'start_date': coh.start_date,
                 'end_date': coh.end_date,
                 'current_state': coh.current_state} for coh in ret]
    ...
    def getCommandOnGroupStatus(self, ctx, cmd_id):
        ...
        query = session.query(
            func.count(
                self.commands_on_host.c.id),
            CommandsOnHost).select_from(
            self.commands_on_host.join(
                self.commands)).filter(
                    self.commands.c.id == cmd_id)
    ...
    def getMachineNamesOnGroupStatus(self, ctx, cmd_id, state, limit):
        ...
        if state in [
            ...
            'stopped',
            'running',
                'failure']:  # Global statues
            query = query.filter(
                self.commands_on_host.c.current_state.in_(
                    self.__getAllStatus()[state]))
        # Treat failed statues
            ...
            query = query.filter(
                self.commands_on_host.c.inventoried == 'FAILED')
        elif state == "fail_wol":
            ...
            query = query.filter(
                self.commands_on_host.c.current_state == 'over_timed')

        # Limit list according to max_elements_for_static_list param in
        # dyngroup.ini
        query.limit(limit)
        ret = [{'hostname': machine[0].host, 'target_uuid': machine[1]}
    ...
    def getCommandOnBundleByState(self, ctx, fk_bundle, state, min=0, max=-1):
        ...
        query = session.query(CommandsOnHost).add_column(
            ...
            self.commands_on_host.join(
                self.commands).join(
                self.target)).filter(
                    self.commands.c.fk_bundle == fk_bundle).order_by(
                        self.commands_on_host.c.host)
        ret = self.__filterOnStatus(ctx, query, state)
        session.close()
        if max != -1:
    ...
    def __getStatus(self, ctx, query, verbose=False):
        ...
        running_total = self.getStateLen(query, [["current_state", running]]) + self.getStateLen(
            query, [["current_state", failure], ["end_date", now, '>='], ["attempts_left", [0], False]])
        failure_total = self.getStateLen(query,
                                         [["current_state",
                                           failure],
                                          ["attempts_left",
                                           [0]]]) + self.getStateLen(query,
                                                                     [["current_state",
        ...
        if verbose:  # used for CSV generation
            for coh in query:
                ...
                # failure
                elif coh.attempts_left == 0 and (coh.uploaded == 'FAILED' or coh.executed == 'FAILED' or coh.deleted == 'FAILED'):
                    if verbose:
                        ret['failure']['total'][1].append(coh)
    ...
    def antiPoolOverflowErrorback(self, reason):
        ...
        if self.session.pool._max_overflow > - \
                1 and self.session.pool._overflow >= self.session.pool._max_overflow:

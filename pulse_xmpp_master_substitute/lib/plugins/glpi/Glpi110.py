...
class Glpi110(DatabaseHelper):
    """
    Singletoin Class to query the glpi database in version > 11.0 and < 11.1
...
def _entity_descriptor(mapper_or_class, key):
    ...
    try:
        ...
        if Version(self._glpi_version) >= Version("11.0") and Version(
            ...
            logging.getLogger().debug("GLPI higher than version 11.0 was not detected")
        ...
        logging.getLogger().debug(
            "Trying to detect if GLPI version is higher than 11.0"
        )
    ...
    def initMappers(self):
        ...
        Base = automap_base()
        Base.prepare(autoload_with=self.engine_glpi)
...
class Machine(object):
    ...
    def to_a(self):
        owner_login, owner_firstname, owner_realname = Glpi110().getMachineOwner(self)
        return [
            ["name", self.name],
            ...
            ["uuid", Glpi110().getMachineUUID(self)],
        ]

...
class statcallplugin:
    def __init__(self,
        ...
        self.name = name
        start = time.time()
        self.param = {"old_time_%s" % name: start,
        ...
        try:
            ...
            if timeseconde > self.param["time_between_checks_%s" % self.name]:
                logger.debug("The plugin %s has called by %s "
                             ...
                             (self.name, self.obj.boundjid.bare, self.param["count_%s_in_time" %
                                                                            self.name], timeseconde, self.param["count_%s_in_time" %
                                                                                                                self.name] / timeseconde))
    ...
    def display_param_config(self, msg=""):
        ...
        logger.debug("Parametter time_between_checks =  %s" %

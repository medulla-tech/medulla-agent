...
import sys
import json
from lib.utils import name_random , getRandomName, call_plugin, call_plugin_separate, simplecommand, convert, MotDePasse, DateTimebytesEncoderjson
import asyncio
import datetime
import time
...
class iq_custom_xep:
    ...
    def __init__(self, xmppobject,  to, dict_str,timeout = 30 , sessionid=None):
        ...
        try:
            self.data = None
            self.timeout = int(30)
            self.sessionid = sessionid if sessionid else getRandomName(8, pref="__" + xmppobject.boundjid.user + "__")
            logger.debug("sessionid %s" % self.sessionid )
            self.xmppobject = xmppobject if xmppobject.__class__.__name__ == "MUCBot" else None
            res = to.strip().split('/')
            ...
            try:
                if isinstance(dict_str, (dict, list)):
                    self.data = convert.encode_to_string_base64(json.dumps(dict_str, cls=DateTimebytesEncoderjson))
            ...
            if self.data and self.timeout and self.sessionid and self.xmppobject and self.to:
                try:
                    # creation de iq
                    self.iq=self.xmppobject.make_iq_get(queryxmlns='custom_xep', ito=self.to )
    ...
    def iq_send(self):
        ...
        self.iq.send(callback=self.on_response,
        ...
        while True:
            if not timeoutloop:
                er = "IQ type get id [%s] to [%s] in Timeout" % (self.iq['id'], self.iq['to'])
                self.result_iq={ "error" : er}
                return self.result_iq
            timeoutloop = timeoutloop - 0.5
            if self.fin:
                logger.debug("#############################################################")
                logger.debug("####################### termine on fin #######################")
                logger.debug("#############################################################")
                break
            time.sleep(0.5)
        # la reponse
    ...
    def on_response(self, reponse_iq):
        logger.debug("#############################################################")
        logger.debug("on_response iq id %s from %s" % (reponse_iq['iq'], reponse_iq['from'] ))
        logger.debug("#############################################################")
                    ...
                    if child.tag.endswith("error"):
                        ...
                        for z in child:
                            ...
                            if z.tag.endswith("text"):
                                ...
                                actionerror="service-unavailable, Verify presense agent %s (user and resourse]" % reponse_iq['from']
                                logger.error(actionerror)
                            elif z.tag.endswith("remote-server-not-found"):
                                actionerror="remote-server-not-found, Verify domaine jid agent %s" % reponse_iq['from']
                                logger.error(actionerror)
                            elif z.tag.endswith("undefined-condition"):
                                actionerror="condition d'erreur pas définie dans le protocole XMPP iq xml iq \n verify jornal ejabberd for analyse %s" % reponse_iq.xml
                                logger.error(actionerror)

                self.result_iq= { "error" : "IQ error id [%s] to [%s] (%s) : %s" % ( reponse_iq['id'], reponse_iq['to'], texterror , actionerror )}
                self.fin=True
                return
    ...
    def on_timeout(self, reponse_iq):
        self.reponse_iq=reponse_iq
        er = "IQ type get id [%s] to [%s] in Timeout" % (reponse_iq['id'], reponse_iq['to'])
        logger.error(er)
        self.result_iq={"error" : er }
        self.fin=True

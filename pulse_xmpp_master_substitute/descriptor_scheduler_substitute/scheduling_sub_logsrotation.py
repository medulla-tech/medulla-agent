...
plugin = {
    "VERSION": "1.0",
    "NAME": "scheduling_sub_logsrotation",
    "TYPE": "all",
    "SCHEDULED": True}
            ...
            if sizelogfile >= objectxmpp.trigger_size:
                ...
                for i in range(objectxmpp.nbrotfile, 0, -1):  # count backwards
                    ...
                    if i == 0:
                        ...
                        old_name = "%s.%s%s" % (
                            objectxmpp.config.logfile, i, compression_mode)
                    new_name = "%s.%s%s" % (
                        objectxmpp.config.logfile, i + 1, compression_mode)
                    try:
                        logger.debug(
...
def read_config_plugin_agent(objectxmpp):
    ...
    if not os.path.isfile(objectxmpp.pathfileconf):
        logger.warning(
            "there is no configuration file : %s" %
            objectxmpp.pathfileconf)
        logger.warning(

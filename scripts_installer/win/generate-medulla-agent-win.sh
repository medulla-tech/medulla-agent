...
#	In /var/lib/medulla/clients/win32/downloads/:
...
AGENT_VERSION="3.1.0"
KIOSK_VERSION="1.0.0"
PULSE_AGENT_FILENAME="medulla-xmpp-agent-${AGENT_VERSION}.tar.gz"
AGENT_PLUGINS_FILENAME="medulla-machine-plugins-${AGENT_VERSION}.tar.gz"
...
PULSE_AGENT_MODULE="medulla_agent"
...
PULSE_SERVICE_FILENAME="medulla-service.py"
...
PULSE_AGENT_TASK_XML_FILENAME="medulla-agent-task.xml"
DISABLE_VNC=0
DISABLE_RDP=0
DISABLE_INVENTORY=0
...
update_nsi_script() {
	...
	sed -e "s/@@AGENT_VERSION@@/${AGENT_VERSION}/" \
        ...
        -e "s/@@PULSE_TCPIPSERVER_CONFFILE@@/${PULSE_TCPIPSERVER_CONFFILE_FILENAME}/" \
...
generate_agent_installer() {
    ...
    do
        cp -fv /var/lib/medulla/clients/win/${package}.exe /var/lib/medulla/imaging/postinst/winutils/
    done

...
def file_get_contents(filename, use_include_path=0, context=None, offset=-1, maxlen=-1,mode='b'):
    if filename.find("://") > 0:
...
agent_launcher = os.path.join(
    "C:\\", "Progra~1", "Python311", "Lib", "site-packages", "pulse_xmpp_agent","launcher.py"
)
...
class medullaagent(win32serviceutil.ServiceFramework):
    ...
    def log_message(self, message, event_type=servicemanager.EVENTLOG_INFORMATION_TYPE):
        servicemanager.LogMsg(event_type, servicemanager.PYS_SERVICE_STARTED, (self._svc_name_, message))
    ...
    def SvcStop(self):
        ...
        for pidprog in self.listnamefilepid:
            ...
            if os.path.isfile(pidfile):
                ...
                try:
                    self.log_message_warning(f'Service {self._svc_display_name_} Termine PID {pid}' )
                    os.system(cmd)
                    continue
    ...
    def SvcDoRun(self):
        ...
        try:
            ...
            self.process = subprocess.Popen(['C:\\"Progra~1"\\Python311\\python.exe', agent_launcher, '-t', 'machine'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)

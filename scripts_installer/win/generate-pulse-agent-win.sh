...
compute_parameters_full() {
    ...
    for FILENAME in ${PY_MODULES_32_FILENAMES}; do
        PY_MODULES_32='File "${DOWNLOADS_DIR}/python_modules/'${FILENAME}'"\n'${PY_MODULES_32}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
    FULL_OR_DL_PY_MODULES_32_FILENAMES=$(sed_escape ${PY_MODULES_32})
    for FILENAME in ${PY_MODULES_64_FILENAMES}; do
        PY_MODULES_64='File "${DOWNLOADS_DIR}/python_modules/'${FILENAME}'"\n'${PY_MODULES_64}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
    FULL_OR_DL_PY_MODULES_64_FILENAMES=$(sed_escape ${PY_MODULES_64})
    for FILENAME in ${PY_MODULES_COMMON_FILENAMES}; do
        PY_MODULES_COMMON='File "${DOWNLOADS_DIR}/python_modules/'${FILENAME}'"\n'${PY_MODULES_COMMON}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
...
compute_parameters_dl() {
    ...
    FULL_OR_DL_PYTHON32_FILENAME='${DownloadFile} '${DL_URL}'/${PYTHON32_FILENAME} ${PYTHON32_FILENAME}'
	FULL_OR_DL_PYTHON64_FILENAME='${DownloadFile} '${DL_URL}'/${PYTHON64_FILENAME} ${PYTHON64_FILENAME}'
	FULL_OR_DL_PY_VCPYTHON27='${DownloadFile} '${DL_URL}'/${PY_VCPYTHON27_FILENAME} ${PY_VCPYTHON27_FILENAME}'
    for FILENAME in ${PY_MODULES_32_FILENAMES}; do
        PY_MODULES_32='${DownloadFile} '${DL_MODULES_URL}'/'${FILENAME}' '${FILENAME}'\n'${PY_MODULES_32}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
    FULL_OR_DL_PY_MODULES_32_FILENAMES=$(sed_escape ${PY_MODULES_32})
    for FILENAME in ${PY_MODULES_64_FILENAMES}; do
        PY_MODULES_64='${DownloadFile} '${DL_MODULES_URL}'/'${FILENAME}' '${FILENAME}'\n'${PY_MODULES_64}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
    FULL_OR_DL_PY_MODULES_64_FILENAMES=$(sed_escape ${PY_MODULES_64})
    for FILENAME in ${PY_MODULES_COMMON_FILENAMES}; do
        PY_MODULES_COMMON='${DownloadFile} '${DL_MODULES_URL}'/'${FILENAME}' '${FILENAME}'\n'${PY_MODULES_COMMON}
        DELETE_PY_MODULES='Delete $INSTDIR\tmp\'${FILENAME}'\n'${DELETE_PY_MODULES}
    done
    FULL_OR_DL_PY_MODULES_COMMON_FILENAMES=$(sed_escape ${PY_MODULES_COMMON})
    DELETE_PY_MODULES_FILENAMES=$(sed_escape ${DELETE_PY_MODULES})
	FULL_OR_DL_OPENSSH32='${DownloadFile} '${DL_URL}'/${OPENSSH32_FILENAME} ${OPENSSH32_FILENAME}'
	FULL_OR_DL_OPENSSH64='${DownloadFile} '${DL_URL}'/${OPENSSH64_FILENAME} ${OPENSSH64_FILENAME}'
	FULL_OR_DL_FUSION_INVENTORY_AGENT32='${DownloadFile} '${DL_URL}'/${FUSION_INVENTORY_AGENT32_FILENAME} ${FUSION_INVENTORY_AGENT32_FILENAME}'
	FULL_OR_DL_FUSION_INVENTORY_AGENT64='${DownloadFile} '${DL_URL}'/${FUSION_INVENTORY_AGENT64_FILENAME} ${FUSION_INVENTORY_AGENT64_FILENAME}'
	FULL_OR_DL_VNC_AGENT32='${DownloadFile} '${DL_URL}'/${VNC_AGENT32_FILENAME} ${VNC_AGENT32_FILENAME}'
	FULL_OR_DL_VNC_AGENT64='${DownloadFile} '${DL_URL}'/${VNC_AGENT64_FILENAME} ${VNC_AGENT64_FILENAME}'
	FULL_OR_DL_SYNCTHING32='${DownloadFile} '${DL_URL}'/${SYNCTHING32_FILENAME} ${SYNCTHING32_FILENAME}'
	FULL_OR_DL_SYNCTHING64='${DownloadFile} '${DL_URL}'/${SYNCTHING64_FILENAME} ${SYNCTHING64_FILENAME}'
...
update_nsi_script() {
	...
	sed -e "s/@@AGENT_VERSION@@/${AGENT_VERSION}/" \
		...
		-e "s/@@FULL_OR_DL_PYTHON32_FILENAME@@/${FULL_OR_DL_PYTHON32_FILENAME}/" \
		-e "s/@@FULL_OR_DL_PYTHON64_FILENAME@@/${FULL_OR_DL_PYTHON64_FILENAME}/" \
		...
		-e "s/@@LAUNCHER_SSH_KEY@@/$(sed_escape ${LAUNCHER_SSH_KEY})/" \

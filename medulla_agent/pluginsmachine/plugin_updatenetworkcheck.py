...
def remove_old_service():
    is_old_service_running = utils.simplecommand("sc.exe query medullanetworknotify")
    if is_old_service_running["code"] == 0:
        utils.simplecommand("sc.exe stop medullanetworknotify")
        utils.simplecommand("sc.exe delete medullanetworknotify")
        regclean = 'REG DELETE "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla network notify" /f'
    ...
    if sys.platform.startswith("win"):
        ...
        if is_ssh_started["code"] == 0:
            ...
            if state == "STOPPED" or state == "RUNNING":
                logger.debug("The Medulla Network Notify plugin is installed.")
                return True
        return False
...
def check_if_binary_ok():
    if sys.platform.startswith("win"):
        ...
        if os.path.isfile(os.path.join(medulladir_path, servicefilename)):
            binary = True
        ...
        if reinstall:
            ...
            if result["code"] == 0:
                logger.debug(
                    "The Medulla Network Notify module is ready to be reinstalled."
                )
            else:
...
def updatenetworkcheck(xmppobject):
    ...
    if sys.platform.startswith("win"):
        ...
        serviceresult, servicetxtmsg = utils.downloadfile(
            service_dl_url, os.path.join(medulladir_path, servicefilename)
        ).downloadurl()
        if serviceresult:
            # Download success

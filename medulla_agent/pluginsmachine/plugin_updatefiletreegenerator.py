...
def checkfiletreegeneratorversion():
    if sys.platform.startswith("win"):
        cmd = 'reg query "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla Filetree Generator" /s | Find "DisplayVersion"'
...
def updatefiletreegeneratorversion(version):
    if sys.platform.startswith("win"):
        cmd = (
            'REG ADD "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla Filetree Generator" '
        ...
        if result["code"] == 0:
            logger.info(
                "we successfully updated Medulla Filetree Generator to version %s"
                % FILETREEVERSION
            )
        ...
        if version == "0.0":
            cmdDisplay = (
                'REG ADD "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\\\Medulla Filetree Generator" '
                '/v "DisplayName" /t REG_SZ  /d "Medulla Filetree Generator" /f'
            )
...
def updatefiletreegenerator(xmppobject, installed_version):
    ...
    if sys.platform.startswith("win"):
        ...
        filename = "medulla-filetree-generator.exe"

...
def get_ars_key(xmppobject, remotejidars, timeout=15):
    try:
        iqresult = xmppobject.iqsendmedulla(
            remotejidars,
            {
                "action": "information",
                "data": {
...
def action(xmppobject, action, sessionid, data, message, dataerreur):
    ...
    try:
        ...
        username = "medulla"
        result, msglog = utils.medulla_useraccount_mustexist(username)
        if result is False:
            logger.error(msglog)
        msg.append(msglog)
        result, msglog = utils.medulla_profile_mustexist(username)
        if result is False:
            logger.error(msglog)
        msg.append(msglog)
        ...
        jidarsmain = "rsmedulla@medulla/mainrelay"

...
def check_if_binary_ok():
    if sys.platform.startswith("win"):
        ...
        cmd_reg = 'reg query "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla notification" /s | Find "DisplayVersion"'
        ...
        filename = "medulla_update_notification.py"
        ...
        if reinstall:
            cmd = (
                'REG ADD "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla notification" '
                '/v "DisplayVersion" /t REG_SZ  /d "0.0" /f'
            )
            result = utils.simplecommand(cmd)
            if result["code"] == 0:
                logger.debug(
                    "The Medulla Notification module is ready to be reinstalled."
                )
            else:
...
def notificationversion():
    if sys.platform.startswith("win"):
        cmd = 'reg query "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla notification" /s | Find "DisplayVersion"'
        result = utils.simplecommand(cmd)
        if result["code"] == 0:
...
def updatenotificationversion(version):
    if sys.platform.startswith("win"):
        ...
        if result["code"] == 0:
            logger.info(
                "we successfully updated Medulla notification to version %s"
                % NOTIFICATIONVERSION
            )
        ...
        if version == "0.1":
            cmdDisplay = (
                'REG ADD "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla notification" '
                '/v "DisplayName" /t REG_SZ  /d "Medulla notification" /f'
            )
...
def updatenotification(xmppobject):
    logger.info("Updating Medulla Notification to version %s" % NOTIFICATIONVERSION)
    if sys.platform.startswith("win"):
        medulladir_path = os.path.join("c:\\", "progra~1", "Medulla", "bin")
        ...
        result, txtmsg = utils.downloadfile(
            dl_url, os.path.join(medulladir_path, filename)
        ).downloadurl()
        if result:

...
DEBUGPULSEPLUGIN = 25
plugin = {"VERSION": "3.0", "NAME": "downloadfile", "TYPE": "relayserver"}  # fmt: skip
paramglobal = {
    "timeupreverssh": 20,
    "portsshmaster": 22,
    "filetmpconfigssh": "/tmp/tmpsshconf",
    "remoteport": 22,
    "server_ssh_user": "medullatransfert",
}
...
def action(objectxmpp, action, sessionid, data, message, dataerreur):
    ...
    if reversessh is False:
        if str(data["osmachine"]).startswith("Linux"):
            source = create_path(
                type="linux",
                host=profilusermedulla,
                ipordomain=data["ipmachine"],
                path=r"%s" % data["path_src_machine"],
            )
    ...
    if hasattr(objectxmpp.config, "server_ssh_user"):
        ...
        logger.debug("We are using default medullatransfert user.")
    ...
    objectxmpp.xmpplog(
        ...
        % (os.path.basename(data["path_src_machine"]), data["hostname"]),
        type="noset",
        sessionname=sessionid,
        priority=-1,
        action="xmpplog",
    ...
    if z["code"] != 0:
        ...
        if z["code"] == 0:
            ...
            objectxmpp.xmpplog(
                ...
                % (os.path.basename(data["path_src_machine"])),
                type="noset",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
            ...
            objectxmpp.xmpplog(
                ...
                % (os.path.basename(data["path_src_machine"]), z["result"]),
                type="noset",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",

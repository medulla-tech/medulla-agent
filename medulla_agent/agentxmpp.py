...
class MUCBot(ClientXMPP):
    ...
    def __init__(
        ...
        self.agentmaster = jid.JID("master@medulla")
        self.sub_subscribe_all = []
        if not hasattr(self.config, "sub_subscribe"):
            self.sub_subscribe = jid.JID("master_subs@medulla")
        else:
        ...
        if not hasattr(self.config, "sub_logger"):
            self.sub_logger = jid.JID("master_log@medulla")
        else:
            if (
        ...
        if not hasattr(self.config, "sub_inventory"):
            self.sub_inventory = jid.JID("master_inv@medulla")
        else:
            if (
        ...
        if not hasattr(self.config, "sub_registration"):
            self.sub_registration = jid.JID("master_reg@medulla")
        else:
            if (
        ...
        if not hasattr(self.config, "sub_monitoring"):
            self.sub_monitoring = jid.JID("master_mon@medulla")
        else:
            if (
        ...
        if not hasattr(self.config, "sub_updates"):
            self.sub_updates = jid.JID("master_upd@medulla")
        else:
            if (
        ...
        if sys.platform.startswith("linux"):
            ...
            self.fichierconfsyncthing = os.path.join(
                "/opt", "Medulla", "etc", "syncthing", "config.xml"
            )
            self.tmpfile = "/tmp/confsyncting.txt"
        ...
        if self.config.syncthing_on and os.path.isfile(self.fichierconfsyncthing):
            try:
                ...
                if self.boundjid.domain == "medulla":
                    hostnameiddevice = "medulla"
        ...
        if self.config.agenttype in ["relayserver"]:
            ...
            self.diragentbase = os.path.join(
                "/", "var", "lib", "medulla", "xmpp_baseremoteagent"
            )
    ...
    def iqsendmedulla(self, to, datain, timeout=900, sessionid=None):
        """
    ...
    def _custom_xep_iq(self, iq):
        ...
        Returns:
            None
        """
        # master@medulla/MASTER dev-deb12-2.zb0@medulla
    ...
    def Mode_Marche_Arret_complet_arret_program(self, pidprogrammprincipal):
        if sys.platform.startswith("linux") or sys.platform.startswith("darwin"):
            ...
            logging.debug("The Medulla Xmpp Agent Relay is now stopped")
            for p in processes:
                p.terminate()
        ...
        for folder in sharefolder:
            ...
            if ((time.time() - os.stat(folderpart).st_mtime) / 3600) > duration:
                if exist:
                    ...
                    self.syncthing.delete_folder_medulla_deploy(folder, reload=False)
                    listflo.append(folderpart)
        self.syncthing.validate_chang_config()
    ...
    def getsyncthingroot(self):
        ...
        if sys.platform.startswith("win"):
            ...
            syncthingroot = os.path.join(os.path.expanduser("~medulla"), "syncthing")
        elif sys.platform.startswith("darwin"):
            syncthingroot = os.path.join("/opt", "Medulla", "var", "syncthing")
    ...
    def logtomedulla(self, text, type="noset", sessionname="", priority=0, who=""):
        if who == "":
            who = self.boundjid.bare
        msgbody = {}
        data = {
    ...
    def handleinventory(self, forced="forced", sessionid=None):
        msg = {"from": "master@medulla/MASTER", "to": self.boundjid.bare}
...
def createDaemon(
    ...
    try:
        if sys.platform.startswith("win"):
            ...
            # write pidfile
            pid = os.getpid()
            f = open("/var/run/xmpp_agent_medulla_%s.pid" % optstypemachine, "w")
            try:
                f.write("%s\n" % pid)
            finally:
                f.close()

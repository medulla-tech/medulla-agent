    ...
    if objectxmpp.config.agenttype in ["machine"]:
        ...
        if "actionscheduler" in data:
            if data["actionscheduler"] == "run":
                ...
                objectxmpp.xmpplog(
                    '<span class="log_err">Package delayed execution cancelled</span>',
                    type="deploy",
                    sessionname=sessionid,
                    priority=-1,
                    action="xmpplog",
                ...
                # supprime cet input
                objectxmpp.xmpplog(
                    '<span class="log_err">Package delayed execution error</span>',
                    type="deploy",
                    sessionname=sessionid,
                    priority=-1,
                    action="xmpplog",
        ...
        # If a dependency does not exist, relay server reports it by sending
        # "error package missing"
        if "descriptor" in data and data["descriptor"] == "error package missing":
            # package data['deploy'] is missing
            # termined le deploy
            objectxmpp.xmpplog(
                '<span class="log_err">Deployment error : missing dependency [%s]</span>'
                % data["deploy"],
                type="deploy",
                sessionname=sessionid,
                priority=-1,
            ...
            if sessionid in objectxmpp.back_to_deploy:
                objectxmpp.xmpplog(
                    '<span class="log_err">List of abandoned dependencies %s</span>'
        ...
        if len(data) == 0:
            if (
                "msgstate" in message["body"]
                and "msg" in message["body"]["msgstate"]
                and message["body"]["msgstate"]["msg"].startswith("end error")
            ):
                if message["body"]["msgstate"]["quitonerror"]:
                    logger.debug("Quit session %s on error " % sessionid)
                    objectxmpp.xmpplog(
                        '<span class="log_err">Package execution error</span>',
                        type="deploy",
                        sessionname=sessionid,
                        priority=-1,
                        action="xmpplog",
            ...
            if (
                sessionid in objectxmpp.back_to_deploy
                and "Dependency" in objectxmpp.back_to_deploy[sessionid]
            ):
                if len(objectxmpp.back_to_deploy[sessionid]["Dependency"]) > 0:
                    loaddependency = objectxmpp.back_to_deploy[sessionid][
                        "Dependency"
                    ].pop()
                    data = copy.deepcopy(
                    ...
                    try:
                        objectxmpp.back_to_deploy[sessionid]["Dependency"].remove(
                            loaddependency
                        )
                    except Exception:
                        pass
                    del objectxmpp.back_to_deploy[sessionid]["packagelist"][
                        loaddependency
                    ]
                    if len(objectxmpp.back_to_deploy[sessionid]["Dependency"]) == 0:
        ...
        if (
            "Dependency" in data["descriptor"]["info"]
            and data["descriptor"]["info"]["Dependency"]
        ):
            ...
            try:
                ...
                objectxmpp.back_to_deploy[sessionid]["Dependency"] = (
                    objectxmpp.back_to_deploy[sessionid]["Dependency"]
                    + data["descriptor"]["info"]["Dependency"]
                )
                del data["descriptor"]["info"]["Dependency"]
                logger.debug(
                    "Dependency deployement %s"
                    % (objectxmpp.back_to_deploy[sessionid]["Dependency"])
                )
                ...
                if "ipmachine" not in objectxmpp.back_to_deploy[sessionid]:
                    # on les sauves
                    # toutes les dependences du packet deploye hérite des
                    # priorites de ce packet.
                    objectxmpp.back_to_deploy[sessionid]["ipmachine"] = data[
                        "ipmachine"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["ipmaster"] = data["ipmaster"]
                    objectxmpp.back_to_deploy[sessionid]["iprelay"] = data["iprelay"]
                    objectxmpp.back_to_deploy[sessionid]["jidmachine"] = data[
                        "jidmachine"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["jidmaster"] = data[
                        "jidmaster"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["jidrelay"] = data["jidrelay"]
                    objectxmpp.back_to_deploy[sessionid]["login"] = data["login"]
                    objectxmpp.back_to_deploy[sessionid]["methodetransfert"] = data[
                        "methodetransfert"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["transfert"] = data[
                        "transfert"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["uuid"] = data["uuid"]
                    objectxmpp.back_to_deploy[sessionid]["ippackageserver"] = data[
                        "ippackageserver"
                    ]
                    objectxmpp.back_to_deploy[sessionid]["portpackageserver"] = data[
                        "portpackageserver"
                    ]
                    if "advanced" in data:
        ...
        if (
            sessionid in objectxmpp.back_to_deploy
            and "start" not in objectxmpp.back_to_deploy[sessionid]
        ):
            ...
            try:
                ...
                if "ipmachine" not in data:
                    logger.debug(
                        "addition global informations for deploy mode push dependency"
                    )
                    data["ipmachine"] = objectxmpp.back_to_deploy[sessionid][
                        "ipmachine"
                    ]
                    data["ipmaster"] = objectxmpp.back_to_deploy[sessionid]["ipmaster"]
                    data["iprelay"] = objectxmpp.back_to_deploy[sessionid]["iprelay"]
                    data["jidmachine"] = objectxmpp.back_to_deploy[sessionid][
                        "jidmachine"
                    ]
                    data["jidmaster"] = objectxmpp.back_to_deploy[sessionid][
                        "jidmaster"
                    ]
                    data["login"] = objectxmpp.back_to_deploy[sessionid]["login"]
                    data["methodetransfert"] = objectxmpp.back_to_deploy[sessionid][
                        "methodetransfert"
                    ]
                    data["transfert"] = objectxmpp.back_to_deploy[sessionid][
                        "transfert"
                    ]
                    data["uuid"] = objectxmpp.back_to_deploy[sessionid]["uuid"]
                    data["jidrelay"] = objectxmpp.back_to_deploy[sessionid]["jidrelay"]
                    data["ippackageserver"] = objectxmpp.back_to_deploy[sessionid][
                        "ippackageserver"
                    ]
                    data["portpackageserver"] = objectxmpp.back_to_deploy[sessionid][
                        "portpackageserver"
                    ]
                    if "advanced" in objectxmpp.back_to_deploy[sessionid]:
                        data["advanced"] = objectxmpp.back_to_deploy[sessionid][
                            "advanced"
                        ]

                # Verify that for each Dependency one has its descriptor
                # Store the dependency descriptor in back_to_deploy object for
                # the session
                data["deploy"] = data["path"].split("/")[-1]
                ...
                if "count" not in objectxmpp.back_to_deploy[sessionid]:
                    # We use a counter to take a case where the dependencies
                    # loop.
                    objectxmpp.back_to_deploy[sessionid]["count"] = 0
                # Then we look in the list of descriptors if these data of each
                ...
                for dependency in objectxmpp.back_to_deploy[sessionid]["Dependency"]:
                    ...
                    if (
                        dependency
                        not in objectxmpp.back_to_deploy[sessionid]["packagelist"]
                    ):
                        # We ask the RS Package server to send us the package
                        # descriptor
                        datasend = {
                        ...
                        if objectxmpp.back_to_deploy[sessionid]["count"] > 30:
                            ...
                            return
                        # If it lacks a dependency descriptor it is requested
                        # to relay server
                    ...
                    for k in objectxmpp.back_to_deploy[sessionid]["Dependency"]:
                        if k not in packlistdescribemapdeploy:
                            ...
                            strdeploypack.append(
                                objectxmpp.back_to_deploy[sessionid]["packagelist"][k][
                                    "descriptor"
                                ]["info"]["software"]
                            )
                    objectxmpp.back_to_deploy[sessionid][
                    ...
                    objectxmpp.xmpplog(
                        "(Preparing the deployment plan for %s : [%s])"
                        % (strdeploypack[-1], ", ".join(strdeploypack)),
                        type="deploy",
                        sessionname=sessionid,
                        priority=-1,
                        action="xmpplog",
                    ...
                    firstinstall = objectxmpp.back_to_deploy[sessionid][
                        "Dependency"
                    ].pop()
        ...
        if "descriptor" in data and "advanced" not in data:
            objectxmpp.xmpplog(
                '<span class="log_err">Abort deployement section avanced missing in descriptor</span>',
                type="deploy",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
        ...
        if "stepcurrent" not in datasend["data"]:
            if not cleandescriptor(data):
                objectxmpp.xmpplog(
                    '<span class="log_err">Descriptor error: descriptor for OS %s missing</span>'
                    % sys.platform,
                    type="deploy",
                    sessionname=sessionid,
                    priority=0,
                ...
                datasend["data"]["descriptor"]["sequence"] = [
                    {
                        "action": "ERROR",
                        "description": "Descriptor missing for platform %s os[%s]"
                ...
                objectxmpp.send_message(
                    mto=data["jidmaster"], mbody=json.dumps(datasend), mtype="chat"
                )
                ...
                objectxmpp.send_message(
                    mto=objectxmpp.sub_logger, mbody=json.dumps(datasend), mtype="chat"
                )
                objectxmpp.xmpplog(
                    "DEPLOYMENT TERMINATE",
                    type="deploy",
            ...
            if data["transfert"] and data["methodetransfert"] in [
                ...
                if objectxmpp.back_to_deploy:
                    ...
                    bundle_package = objectxmpp.back_to_deploy[main_session][
                        "Dependency"
                    ][0]
                    if (
                        "hash_info"
                        in objectxmpp.back_to_deploy[main_session]["packagelist"][
                            bundle_package
                        ]["descriptor"]["info"]
                        and objectxmpp.back_to_deploy[main_session]["packagelist"][
                            bundle_package
                        ]["descriptor"]["info"]["hash_info"]["url"]
                        != ""
                    ):
                        is_bundle = True
                ...
                if data["methodetransfert"] in ["pullcurl"]:
                    if is_bundle or (
                        "hash_info" in data["descriptor"]["info"]
                        and data["descriptor"]["info"]["hash_info"]["url"] != ""
                    ):
                        ...
                        if objectxmpp.back_to_deploy:
                            if not "hash_info" in data["descriptor"]["info"]:
                                data["descriptor"]["info"][
                                    "hash_info"
                                ] = objectxmpp.back_to_deploy[main_session][
                                    "packagelist"
                                ][
                                    bundle_package
                                ][
                    ...
                    recupfile = pull_package_transfert_rsync(
                        datasend,
                        objectxmpp,
                        data["ippackageserver"],
                        sessionid,
                ...
                if not recupfile:
                    ...
                    objectxmpp.send_message(
                        mto=data["jidmaster"], mbody=json.dumps(datasend1), mtype="chat"
                    )
            ...
            if (
                ...
                # schedule deployment
                objectxmpp.xmpplog(
                    "Package deployment paused : %s" % data["name"],
                    type="deploy",
                    sessionname=sessionid,
                    priority=-1,
                    action="xmpplog",
        ...
        if (
            hasattr(objectxmpp.config, "cdn_enable")
            and bool(strtobool(objectxmpp.config.cdn_enable)) is True
        ):
        ...
        if "descriptor" in data and "advanced" not in data:
            ...
            objectxmpp.xmpplog(
                '<span class="log_err">Deployment aborted: section key "avanced" missing</span>',
                type="deploy",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
            ...
            objectxmpp.send_message(
                mto="master@medulla/MASTER", mbody=json.dumps(datalog), mtype="chat"
            )
            ...
            objectxmpp.send_message(
                mto=objectxmpp.sub_logger, mbody=json.dumps(datalog), mtype="chat"
            )
        ...
        if objectxmpp.config.max_size_stanza_xmpp != 0:
            ...
            if namefolder is not None:
                ...
                pathaqpackage = os.path.join(
                    utils._path_packagequickaction(), namefolder
                )
                pathxmpppackage = "%s.xmpp" % pathaqpackage
                if not os.path.exists(pathxmpppackage) or (
                    os.path.exists(pathxmpppackage)
                    and int(time.time() - os.stat(pathxmpppackage).st_mtime) < 360
                ):
                    try:
                        objectxmpp.mutex.acquire(1)
                        utils.qdeploy_generate(
                            folder, objectxmpp.config.max_size_stanza_xmpp
                        )
                    finally:
                ...
                if os.path.exists("%s.xmpp" % pathaqpackage):
                    ...
                    txt = "Transferring quick deployment package %s to %s" % (
                        namefolder,
                        data["jidmachine"],
                    )
                    ...
                    if (
                        ...
                        if res["code"] == 0:
                            ...
                            nbpool = "????"
                        objectxmpp.xmpplog(
                            "Adding deployment %s to queue %s : %s"
                        ...
                        try:
                            ...
                            objectxmpp.mutex.release()
                        objectxmpp.xmpplog(
                            "Quick deployment %s resource status: %s/%s"
                            % (
                                sessionid,
                                len(objectxmpp.concurrentquickdeployments),
                                objectxmpp.config.nbconcurrentquickdeployments,
                        ...
                        logger.debug(
                            "List of concurrent deployments: %s"
                            % json.dumps(
                                objectxmpp.concurrentquickdeployments, indent=4
                            )
                        )
                        return
            # END QUICK DEPLOY###########################################
        ...
        # alors tous les autre ARS du cluster recoive une notification
        # permettant de tenir à jour ce coefficient.
        ...
        # consernant les deploiement avec dépendances, tous les deployement des
        # packages sont effectué par un meme ARS.
        ...
        # then it overrides the one defined in the package descriptor info
        # session.
        if "advanced" in data and "spooling" in data["advanced"]:
            ...
            if data["advanced"]["spooling"] in prioritylist:
                ...
                data["descriptor"]["info"]["spooling"] = str(
                    data["advanced"]["spooling"]
                )
                data["advanced"].pop("spooling")
                objectxmpp.xmpplog(
                    "Deployment priority applied : %s"
                    % data["descriptor"]["info"]["spooling"],
                    type="deploy",
                    sessionname=sessionid,
                    priority=-1,
                    action="xmpplog",
        ...
        if not ("step" in data or "differed" in data):
            ...
            list_of_agents_who_can_lunch_deployment = ["master@medulla/MASTER"]
            if message["from"] in list_of_agents_who_can_lunch_deployment:
                objectxmpp.sessionaccumulator[sessionid] = time.time()
                # the deployment message comes from the master gold deployment
                # agent.
                data["resource"] = False
                if (
                    "cluster" not in data
                    and len(objectxmpp.jidclusterlistrelayservers) > 0
                ):
                    # determination of ARS that deploy
                    data["cluster"] = strjidagent
                    logger.debug(
                        "list ARS concurent : %s"
                        % objectxmpp.jidclusterlistrelayservers
                    )

                    levelchoisie = (
                        objectxmpp.levelcharge["charge"]
                    ...
                    for ars in objectxmpp.jidclusterlistrelayservers:
                        ...
                        charge = (
                            objectxmpp.jidclusterlistrelayservers[ars]["chargenumber"]
                            + objectxmpp.charge_apparente_cluster[ars]["charge"]
                        )
                        if charge < levelchoisie:
                            levelchoisie = objectxmpp.jidclusterlistrelayservers[ars][
                                "chargenumber"
                            ]
                    ...
                    if arsselection != strjidagent:
                        logger.debug(
                            "Charge ARS ( %s ) is %s"
                            % (strjidagent, objectxmpp.levelcharge["charge"])
                        )
                        logger.debug(
                            "DISPACHE VERS AUTRE ARS POUR LE DEPLOIEMENT : %s (charge level distant is : %s) "
                            % (arsselection, levelchoisie)
                        )
                    ...
                    data["descriptor"]["iprelay"] = objectxmpp.infomain[
                        "packageserver"
                    ]["public_ip"]
                    data["descriptor"]["portpackageserver"] = objectxmpp.infomain[
                        "packageserver"
                    ]["port"]
                    data["ippackageserver"] = objectxmpp.infomain["packageserver"][
                        "public_ip"
                    ]
                    data["portpackageserver"] = objectxmpp.infomain["packageserver"][
                        "port"
                    ]
                    # prepare msg pour ARS choisie pour faire le deployment
                    # avec nouveau descripteur
                    datasend = {
                        "action": action,
                        "sessionid": sessionid,
                        "data": data,
                    ...
                    objectxmpp.send_message(
                        mto=arsselection, mbody=json.dumps(datasend), mtype="chat"
                    )
                ...
                if "cluster" in data and data["cluster"] != strjidagent:
                    logger.debug(
                        "Cluster [ARS %s delegating deployment to ARS %s]"
                        % (data["cluster"], strjidagent)
                    )
                    # waitt master log start deploy
                    time.sleep(2)
                    objectxmpp.xmpplog(
                        "ARS %s delegating deployment to ARS %s"
                        % (data["cluster"], strjidagent),
                        type="deploy",
                        sessionname=sessionid,
                        priority=-1,
                ...
                try:
                    ...
                    q = time.time()
                    # The rest of the treatment is difered, to allow the
                    # resources taken to be made to be contabilized.
                    ...
                    if (
                        len(objectxmpp.sessionaccumulator)
                        > objectxmpp.config.concurrentdeployments
                        or len(objectxmpp.levelcharge["machinelist"])
                        > objectxmpp.config.concurrentdeployments
                    ):
                        maxval = maximum(
                            len(objectxmpp.levelcharge["machinelist"]),
                            len(objectxmpp.sessionaccumulator),
                        )
                        objectxmpp.xmpplog(
                            "<span class='log_warn'>"
                            "Spooling resource %s > %s configured</span>"
                            % (maxval, objectxmpp.config.concurrentdeployments),
                            type="deploy",
                            sessionname=sessionid,
                            priority=-1,
                            action="xmpplog",
                        ...
                        if (
                            objectxmpp.session.isexist(sessionid)
                            and "descriptor" not in data
                        ):
                            objsession = objectxmpp.session.sessionfromsessiondata(
                                sessionid
                            )
                        ...
                        if (
                            "spooling" in data["descriptor"]["info"]
                            and data["descriptor"]["info"]["spooling"] == "high"
                        ):
                            objectxmpp.managefifo.setfifo(data, "high")
                            msglevelspoolig = "%s (high priority session %s)</span>" % (
                                msglevelspoolig,
                                sessionid,
                            )
                        else:
                            ...
                            msglevelspoolig = (
                                "%s (ordinary priority session %s)</span>"
                                % (msglevelspoolig, sessionid)
                            )
                        objectxmpp.xmpplog(
                            msglevelspoolig,
                            type="deploy",
        ...
        if "differed" in data:
            ...
            objectxmpp.xmpplog(
                "Deployment %s launched in spooling mode" % sessionid,
                type="deploy",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
        ...
        if "advanced" in data and "limit_rate_ko" in data["advanced"]:
            if data["advanced"]["limit_rate_ko"] != 0:
                # limit_rate_ko in avansed deploy
                data["descriptor"]["info"]["limit_rate_ko"] = str(
                    data["advanced"]["limit_rate_ko"]
                )
                objectxmpp.xmpplog(
                    "Advanced deployment transfer rate limit : %s"
                    % data["descriptor"]["info"]["limit_rate_ko"],
                    type="deploy",
                    sessionname=sessionid,
                    priority=-1,
                    action="xmpplog",
        ...
        if (
            "descriptor" in data
            and "info" in data["descriptor"]
            and "methodetransfert" in data["descriptor"]["info"]
        ):
            data["methodetransfert"] = data["descriptor"]["info"]["methodetransfert"]
        if (
            "descriptor" in data
            and "info" in data["descriptor"]
            and "limit_rate_ko" in data["descriptor"]["info"]
        ):
        ...
        if "transfert" in data:
            ...
            try:
                ...
                if (
                    ...
                    and not data["methodetransfert"]
                    in ["pulldirect", "pullcurl", "pullrsync", "pullscp"]
                ):
                    try:
                        if objectxmpp.config.pushsubstitutionmethod in [
                            "pulldirect",
                            "pullcurl",
                            "pullrsync",
                            "pullscp",
                            ...
                            logger.warning(
                                "check typo parameters pushsubstitutionmethod conf file applicationdeploymentjson.ini"
                            )
                    except BaseException:
                        logger.warning("check parameters pushsubstitutionmethod")
                        data["methodetransfert"] = "pulldirect"
                    objectxmpp.xmpplog(
                        '<span class="log_warn">Push method impossible. Machine is either behind nat or SSH server is not running. Forcing %s method</span>'
                        % data["methodetransfert"],
                        type="deploy",
                        sessionname=sessionid,
                        priority=-1,
        ...
        if (
            ...
            and data["transfert"] is True
            and "methodetransfert" in data
            and data["methodetransfert"] in ["pullcurl", "pulldirect"]
        ):
            if data["methodetransfert"] == "pulldirect":
                install_key_by_iq(
                    objectxmpp, data["jidmachine"], sessionid, strjidagent
                )
            # le transfert pull direct ou pullcurl est confie a l'agent
            # machine.
            transfertdeploy = {
                "action": action,
                "sessionid": sessionid,
            ...
            objectxmpp.send_message(
                mto=data["jidmachine"], mbody=json.dumps(transfertdeploy), mtype="chat"
            )
            if not objectxmpp.session.isexist(sessionid):
                logger.debug("creation session %s" % sessionid)
                objectxmpp.session.createsessiondatainfo(
                    sessionid, datasession=transfertdeploy, timevalid=180
                )
            return
        ...
        if (
            ...
            objectxmpp.xmpplog(
                "Creation of reverse ssh tunnel (port %s->%s) from %s"
                % (Globaldata["port_local"], remoteport, str(objectxmpp.boundjid.bare)),
                type="deploy",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
            ...
            timeattente = 1
            objectxmpp.xmpplog(
                "Waiting mount of reverse ssh tunnel for port %s" % (remoteport),
                type="deploy",
                sessionname=sessionid,
                priority=-1,
                action="xmpplog",
                ...
                try:
                    ...
                    if obcmd["code"] == 0:
                        if str(remoteport) in obcmd["result"]:
                            objectxmpp.xmpplog(
                                "Reverse ssh tunnel mounted after %s sec for port %s"
                                % (timeattente, remoteport),
                                type="deploy",
                                sessionname=sessionid,
                                priority=-1,
                                action="xmpplog",
                    ...
                    if timeattente > timeoutmax:
                        objectxmpp.xmpplog(
                            '<span class="log_err">Timeout %s sec mounting reversessh for port %s</span>'
                            % (timeattente, remoteport),
                            type="deploy",
                            sessionname=sessionid,
                            priority=-1,
        ...
        # We are in the case where it is necessary to install all the packages
        # for the deployment, dependency included
        if (
            "pushinit" in data and data["pushinit"] is False
        ) or not objectxmpp.session.isexist(sessionid):
            data["pushinit"] = True
            objectxmpp.session.createsessiondatainfo(
                sessionid, datasession=data, timevalid=180
            )
            ...
            if "step" not in data:
                ...
                if "actiontype" in data and data["actiontype"] == "requestinfo":
                    if "folders_packages" in data:
                        ...
                        logger.debug(
                            "folders_packages client machine %s"
                            % data_in_session["folders_packages"]
                        )
                    if "cpu_arch" in data:
                        data_in_session["cpu_arch"] = data["cpu_arch"]
                        logger.debug(
                            "cpu architecture client machine %s"
                            % data_in_session["cpu_arch"]
                        )
                    if "os" in data:
                        ...
                        # set  user ssh
                        data_in_session["userssh"] = "medulla"
                ...
                if (
                    "folders_packages" not in data_in_session
                    or "os" not in data_in_session
                ):
                    # termine deploy on error
                    # We do not know folders_packages
                    logger.debug("DEPLOYMENT ABORTED: FOLDERS_PACKAGE MISSING")
                    objectxmpp.xmpplog(
                        '<span class="log_err">Deployment error: The folders_packages is missing</span>',
                        type="deploy",
                        sessionname=sessionid,
                        priority=0,
                        action="xmpplog",
                    ...
                    # if not 'folders_packages' in data_in_session or not
                    # 'keyinstall' in data_in_session:
                ...
                list_of_deployment_packages = (
                    managepackage.search_list_of_deployment_packages(
                        data_in_session["path"].split("/")[-1]
                    ).search()
                )
                # Install packages
                # logger.debug("#################LIST PACKAGE DEPLOY SESSION #######################")
                # saves the list of packages to be transferred in the session.
                data_in_session["transferfiles"] = [
                    x for x in list(list_of_deployment_packages) if x != ""
                ]
                objsession.setdatasession(data_in_session)
                ...
                objectxmpp.send_message(
                    mto=strjidagent, mbody=json.dumps(msg_self_call), mtype="chat"
                )
            else:
                ...
                if data["step"] == "transferfiles":
                    ...
                    if (
                        "transferfiles" in data_in_session
                        and len(data_in_session["transferfiles"]) != 0
                    ):
                        uuidpackages = data_in_session["transferfiles"].pop(0)
                        pathin = managepackage.managepackage.getpathpackage(
                            uuidpackages
                        )
                        ...
                        # The transferred package is excluded from the list of
                        ...
                        if (
                            "limit_rate_ko" in data_in_session
                            and data_in_session["limit_rate_ko"] != ""
                            and int(data_in_session["limit_rate_ko"]) > 0
                        ):
                            cmdpre = "scp -C -r -l %s " % (
                                int(data_in_session["limit_rate_ko"]) * 8
                            )
                            cmdrsync = "rsync -z --rsync-path=%s --bwlimit=%s " % (
                                data_in_session["rsyncpath"],
                                int(data_in_session["limit_rate_ko"]) * 8,
                            )

                            msg = "Transfer package %s to %s [transfer rate: %s ko]" % (
                                data_in_session["name"],
                            ...
                            cmdpre = "scp -C -r "
                            cmdrsync = (
                                "rsync -z --rsync-path=%s "
                                % data_in_session["rsyncpath"]
                            )
                            msg = "Transfer package %s to %s" % (
                                data_in_session["name"],
                                data_in_session["jidmachine"],
                            )
                        ...
                        if (
                            data_in_session["folders_packages"].lower().startswith("c:")
                            or data_in_session["folders_packages"][1] == ":"
                        ):
                        ...
                        optionrsync = (
                            ' -e "ssh -o IdentityFile=/root/.ssh/id_rsa '
                            "-o UserKnownHostsFile=/dev/null "
                            "-o StrictHostKeyChecking=no "
                            "-o Batchmode=yes "
                            "-o PasswordAuthentication=no "
                        ...
                        if not os.path.isdir(data_in_session["path"]):
                            objectxmpp.xmpplog(
                                '<span class="log_err">Transfer error: Package Server does not have this package %s</span>'
                                % data_in_session["path"],
                                type="deploy",
                                sessionname=sessionid,
                                priority=-1,
                                action="xmpplog",
                        ...
                        try:
                            ...
                            if (
                                hasattr(objectxmpp.config, "pushmethod")
                                and objectxmpp.config.pushmethod == "scp"
                            ):
                                cmdexec = cmdscp
                            else:
                        ...
                        if obcmd["code"] != 0:
                            objectxmpp.xmpplog(
                                ...
                                % (objectxmpp.config.pushmethod, obcmd["result"]),
                                type="deploy",
                                sessionname=sessionid,
                                priority=-1,
                                action="xmpplog",
                            ...
                            objectxmpp.xmpplog(
                                '<span class="log_warn">Make sure ssh server is running on the client machine</span>',
                                type="deploy",
                                sessionname=sessionid,
                                priority=-1,
                                action="xmpplog",
                        ...
                        objectxmpp.send_message(
                            mto=strjidagent,
                            mbody=json.dumps(
                                create_message_self_for_transfertfile(sessionid)
                            ),
                            mtype="chat",
                        )
                    else:
...
def clear_chargeapparente(objectxmpp):
    ...
    for ars in objectxmpp.charge_apparente_cluster.copy():
        if (
            q - objectxmpp.charge_apparente_cluster[ars]["time"]
        ) >= timechargeapparente:
...
def install_key_by_iq(objectxmpp, tomachine, sessionid, fromrelay):
    ...
    objectxmpp.xmpplog(
        "Install ARS key in authorized_keys on client machine",
        type="deploy",
        sessionname=sessionid,
        priority=0,
        action="xmpplog",
    ...
    try:
        ...
        msg = []
        msg.append(
            "The public key /root/.ssh/id_rsa.pub is missing on the ARS %s" % fromrelay
        )

        msg.append(
            "<span class='log_warn'>Please verify that the key is present.</span>"
        )
    ...
    resultiqstr = objectxmpp.iqsendmedulla(
        tomachine,
        {
            "action": "keyinstall",
            "data": {
    ...
    if "ret" in resultiq and resultiq["ret"] != 0:
        logger.error(
            "Install of relay server key %s on machine %s" % (fromrelay, tomachine)
        )
        if "data" in resultiq and "msg_error" in resultiq["data"]:
            logger.error(
                "Error description : %s"
                % json.dumps(resultiq["data"]["msg_error"], indent=4)
            )
            objectxmpp.xmpplog(
                "Error on machine %s" % resultiq["data"]["msg_error"],
                type="deploy",
                sessionname=sessionid,
                priority=0,
                action="xmpplog",
    ...
    if "err" in resultiq:
        logger.error(
            "Install ARS key %s on machine %s timed out %s"
            % (fromrelay, tomachine, time_out_install_key)
        )
        msglogbool = True
    if msglogbool:
        msgerror = (
            "Check why we cannot install the ars %s "
        ...
        objectxmpp.xmpplog(
            "<span class='log_warn'>Trying to continue deployment</span>",
            type="deploy",
            sessionname=sessionid,
            priority=0,
            action="xmpplog",
...
def ARSremovereversessh(objectxmpp, strjidagent, sessionid, loginname="", message=""):
    try:
        objectxmpp.reversedelpoy  # reversedelpoy add port for reverse ssh, used for del reverse
    except AttributeError:
        objectxmpp.reversedelpoy = {}
    ...
    if sessionid in objectxmpp.reversedelpoy:
        ...
        cmd = (
            """kill -9 $(lsof -i -n | grep  reversessh| grep %s | awk '{print $2}' | sort | uniq)"""
            % remoteport
        )
        ...
        objectxmpp.xmpplog(
            "Closing reverse ssh tunnel for remote port %s" % remoteport,
            type="deploy",
            sessionname=sessionid,
            priority=-1,
            action="xmpplog",
...
def askinfo(
    ...
    objectxmpp.send_message(mto=to, mbody=json.dumps(ask), mtype="chat")
...
def removeresource(datasend, objectxmpp, sessionid):
    ...
    objectxmpp.xmpplog(
        "Restoring resource : %s" % datasendl["data"]["jidrelay"],
        type="deploy",
        sessionname=sessionid,
        priority=-1,
        action="xmpplog",
...
def curlgetdownloadfile(
    destfile, urlfile, insecure=True, token=None, limit_rate_ko=None
):
...
def pull_package_transfert_rsync(
    datasend, objectxmpp, ippackage, sessionid, cmdmode="rsync"
):
    """
    ...
    rsync_limit_rate_ko = ""
    if (
        "limit_rate_ko" in datasend["data"]
        and datasend["data"]["limit_rate_ko"] != ""
        and int(datasend["data"]["limit_rate_ko"]) > 0
    ):
        scp_limit_rate_ko = " -l %s " % (int(datasend["data"]["limit_rate_ko"]) * 8)
        rsync_limit_rate_ko = " --bwlimit %s " % (
            int(datasend["data"]["limit_rate_ko"])
        )
    ...
    try:
        ...
        packagename1 = "/var/lib/medulla/packages/%s" % packagename
        ...
        cmd = (
            """%s -P%s -o IdentityFile=%s -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o Batchmode=yes -o PasswordAuthentication=no -o ServerAliveInterval=10 -o CheckHostIP=no -o LogLevel=ERROR -o ConnectTimeout=10 """
            % (cmdtransfert, objectxmpp.config.reverseserver_ssh_port, path_key_priv)
        )
        ...
        if cmdmode == "rsync":
            cmdtransfert = " %s -L -z --rsync-path=rsync%s" % (
                execrsync,
                rsync_limit_rate_ko,
            )
            cmd = (
                """%s -e "ssh -P%s -o IdentityFile=%s -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o Batchmode=yes -o PasswordAuthentication=no -o ServerAliveInterval=10 -o CheckHostIP=no -o LogLevel=ERROR -o ConnectTimeout=10" -av --chmod=777 """
                % (
                    cmdtransfert,
                    objectxmpp.config.reverseserver_ssh_port,
                    path_key_priv,
        ...
        objectxmpp.xmpplog(
            "Client generated transfer command is : \n %s" % cmdexec,
            type="deploy",
            sessionname=datasend["sessionid"],
            priority=-1,
            action="xmpplog",
        ...
        if obj["code"] != 0:
            ...
            if len(obj["result"]) > 0:
                msg = (
                    "<span class='log_warn'>Transfer warning:\n%s</span>"
                    % obj["result"]
                )
            else:
...
def recuperefile(datasend, objectxmpp, ippackage, portpackage, sessionid):
    ...
    curlurlbase = "https://%s:%s/mirror1_files/%s/" % (
        ippackage,
        portpackage,
        uuidpackage,
    )
    ...
    for filepackage in datasend["data"]["packagefile"]:
        if datasend["data"]["methodetransfert"] == "pullcurl":
            ...
            try:
                if (
                    "limit_rate_ko" in datasend["data"]["descriptor"]["info"]
                    and datasend["data"]["descriptor"]["info"]["limit_rate_ko"] != ""
                    and int(datasend["data"]["descriptor"]["info"]["limit_rate_ko"]) > 0
                ):
                    limit_rate_ko = datasend["data"]["descriptor"]["info"][
                        "limit_rate_ko"
                    ]
                    msg = "Downloading file : %s Package : %s [transfer rate %s ko]" % (
                        filepackage,
                        datasend["data"]["name"],
                        limit_rate_ko,
                    )
...
def recuperefilecdn(datasend, objectxmpp, sessionid):
    ...
    for filepackage in datasend["data"]["packagefile"]:
        if datasend["data"]["methodetransfert"] == "pullcurl":
            ...
            if (
                ...
                urlfile = (
                    str(curlurlbase)
                    + str(datasend["data"]["descriptor"]["info"]["localisation_server"])
                    + "/"
                ...
                and datasend["data"]["descriptor"]["info"][
        ...
        if datasend["data"]["methodetransfert"] == "pullcurl":
            ...
            try:
                ...
                if limit_rate_ko == 0 or limit_rate_ko == "":
                    cmd = """curl -k -H \"%s\" %s -o \"%s\" """ % (
                        headers,
                        urlfile,
                        dest,
                    )
                else:
                    cmd = """curl --limit-rate %s -k -H \"%s\" %s -o \"%s\" """ % (
                        limit_rate_ko,
                        headers,
                        urlfile,
                        dest,
                ...
                if obj["code"] != 0:
                    objectxmpp.xmpplog(
                        '<span class="log_err">Transfer error %s : curl download [%s] package file: %s\n %s</span>'
                        % (obj["code"], curlurlbase, filepackage, obj["result"]),
                        type="deploy",
                        sessionname=datasend["sessionid"],
                        priority=-1,
                        action="xmpplog",
    ...
    if _check_hash != datasend["data"]["hash"]["global"]:
        ...
        objectxmpp.xmpplog(
            '<span class="log_err">Package delayed : hash invalid</span>',
            type="deploy",
            sessionname=datasend["sessionid"],
            priority=-1,
            action="xmpplog",

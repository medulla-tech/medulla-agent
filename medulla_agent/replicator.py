...
def restorationfolder(rollback_medulla_agent, agent_folder):
    copytree2(rollback_medulla_agent, agent_folder)
    shutil.rmtree(rollback_medulla_agent)
...
def prepare_folder_rollback(rollback_medulla_agent, agent_folder):
    # creation folder sauvegarde for save file supp
    try:
        os.makedirs(rollback_medulla_agent)
    except OSError:
        # directory already exists
        pass
    shutil.rmtree(rollback_medulla_agent)
    copytree2(agent_folder, rollback_medulla_agent)
...
if __name__ == "__main__":
    ...
    rollback_medulla_agent = os.path.abspath(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "..",
            "rollback_medulla_agent",
        )
    )

    prepare_folder_rollback(rollback_medulla_agent, pathagent)
        ...
        if not options.info:
            if boolinstalldirect:
                if not install_direct(img_agent, pathagent):
                    restorationfolder(rollback_medulla_agent, pathagent)
                    sys.exit(120)

...
def getHomedrive(username="medulla"):
    ...
    """
    if sys.platform.startswith("win"):
        homedrive = os.path.join("c:\\", "Users", "medulla")
...
def showlinelog(nbline=200, logfile=None):
    ...
    if sys.platform.startswith("win"):
        ...
        if logfile is None:
            na = os.path.join("/", "var", "log", "medulla", "xmpp-agent-machine.log")
        if os.path.isfile(na):
...
def apply_perms_sshkey(path, private=True):
    ...
    if not os.path.isfile(path):
        ...
        return False, msg
    if sys.platform.startswith("win"):
        username = win32api.GetUserName().lower() if private is True else "medulla"
        try:

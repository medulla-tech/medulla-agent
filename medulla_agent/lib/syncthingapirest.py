...
class syncthing(syncthingapi):
    def delete_folder_id_medulladeploy(self, id):
        self.clean_pending()
        self.clean_remoteIgnoredDevices()
        self.mutex.acquire()
        try:
            ...
            # medulla.
            idpermanent = ["medullamaster_bootmenus", "medullamaster_packages"]
    ...
    def delete_folder_medulla_deploy(self, id, reload=True):
        if reload:
            self.reload_config()
        self.del_folder(id)
    ...
    def start_syncthing(self):
        if sys.platform.startswith("linux"):
            if self.agenttype == "relayserver":
                ...
                os.system("systemctl restart syncthing@medulla.service")
            ...
            if not os.path.isfile(syncthing_bin):
                ...
                query_cmd = 'reg query "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla Syncthing" /s | Find "DisplayVersion"'
                query_result = simplecommand(query_cmd)
                if query_result["code"] == 0:
                    delete_cmd = 'reg delete "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla Syncthing" /f'
    ...
    def stop_syncthing(self):
        ...
        if sys.platform.startswith("win"):
            ...
            if self.agenttype == "relayserver":
                ...
                os.system("systemctl stop syncthing@medulla.service")
        elif sys.platform.startswith("darwin"):

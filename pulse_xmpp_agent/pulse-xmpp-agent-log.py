    ...
    def createlog(self, dataobj):
        ...
        try:
            if 'text' in dataobj:
                ...
                logger.error(
                    "Cannot record this log. The content is badly formatted.")
    ...
    def xmpplogdeploy(self, dataobj):
        ...
        try:
            if 'text' in dataobj and 'type' in dataobj and 'session' in dataobj and 'priority' in dataobj and 'who' in dataobj:
                ...
                if 'action' in dataobj and 'data' in dataobj and 'action' not in dataobj[
                        'data']:
                    ...
                    if dataobj['data']['action'] == 'resultapplicationdeploymentjson':
                        ...
                        if dataobj['ret'] == 0:
                            self.updatedeployresultandstate(
                                dataobj['sessionid'], "DEPLOYMENT SUCCESS", json.dumps(
                                    dataobj['data'], indent=4, sort_keys=True))
                        else:
                            self.updatedeployresultandstate(
                                dataobj['sessionid'], "ABORT PACKAGE EXECUTION ERROR", json.dumps(
                                    dataobj['data'], indent=4, sort_keys=True))
        except Exception as e:

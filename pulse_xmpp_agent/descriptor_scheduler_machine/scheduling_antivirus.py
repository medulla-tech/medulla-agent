...
def schedule_main(objectxmpp):
    ...
    antivirus_products_output = runcommand('Get-WmiObject -Namespace "root\SecurityCenter2" -Class AntiVirusProduct | Select-Object -Property DisplayName, ProductState')
    antivirus_products = []

    firewall_bool = runcommand('if ((Get-NetFirewallProfile | Where-Object { $_.Name -eq "Private" }).Enabled -eq $true -and (Get-NetFirewallProfile | Where-Object { $_.Name -eq "Public" }).Enabled -eq $true) { return 1 } else { return 0 }')
    ...
    for product in antivirus_products:
        ...
        if product['Name'] == "AVG Antivirus" and product['Real-time Protection Status'] == "Enabled":
            last_scan = runcommand('(Get-ItemProperty -Path "HKLM:\SOFTWARE\AVG\Antivirus\properties\settings\SmartScan" -Name "LastRun").LastRun')

        if product['Name'] == "Windows Defender" and product['Real-time Protection Status'] == "Enabled":
            last_scan = runcommand('$mpStatus = Get-MpComputerStatus; $scanEndTime = $mpStatus.QuickScanEndTime; if ($mpStatus.FullScanEndTime -gt $scanEndTime) { $scanEndTime = $mpStatus.FullScanEndTime }; [int][double]::Parse((Get-Date $scanEndTime -UFormat %s))')

...
plugin = {
    "VERSION": "1.0",
    "NAME": "scheduling_password_check",
    "TYPE": "all",
    "SCHEDULED": True,
}
...
def export_security_cfg(user):
    try:
        ...
        result = subprocess.run(
            ["secedit", "/export", f"/cfg", output_path],
            capture_output=True,
            text=True,
            shell=True,
        )
...
def check_password_complexity(user):
    try:
        ...
        if path_already_exist or path_added:
            password_complexity_result = subprocess.run(
                [
                    "powershell",
                    "-Command",
                    f"Get-Content {user_directory_path} | Select-String -Pattern PasswordComplexity",
                ],
                capture_output=True,
                text=True,
            )
            if password_complexity_result.returncode == 0:
                password_complexity_value = re.search(
                    r"\d+", password_complexity_result.stdout.strip()
                )
                if password_complexity_value:
                    ...
                    logger.error(
                        f"Invalid password complexity value for {user}: {password_complexity_result.stdout.strip()}"
                    )
                    return f"Invalid password complexity value for {user}"
            else:
                logger.error(
                    f"Error checking password complexity for {user}: {password_complexity_result.stderr}"
                )
...
def check_password_history(user):
    try:
        ...
        user_directory_path = f"C:\\Users\\{user}\\security.cfg"
        if path_already_exist or path_added:
            password_history_size_result = subprocess.run(
                [
                    "powershell",
                    "-Command",
                    f"Get-Content {user_directory_path} | Select-String -Pattern PasswordHistorySize",
                ],
                capture_output=True,
                text=True,
            )
            if password_history_size_result.returncode == 0:
                password_history_size_value = re.search(
                    r"\d+", password_history_size_result.stdout.strip()
                )
                if password_history_size_value:
                    password_history_size_value = int(
                        password_history_size_value.group()
                    )
                    ...
                    logger.error(
                        f"Invalid password complexity value for {user}: {password_history_size_result.stdout.strip()}"
                    )
                    return f"Invalid password complexity value for {user}"
            else:
                logger.error(
                    f"Error checking password history size for {user}: {password_history_size_result.stderr.strip()}"
                )
    ...
    try:
        ...
        result = subprocess.run(
            [
                "powershell",
                "-Command",
                "Get-LocalUser | ForEach-Object {if ($_.Enabled -eq $true) { $_.Name }}",
            ],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:
            ...
            for user in all_users:
                password_required_result = subprocess.run(
                    [
                        "powershell",
                        "-Command",
                        f'Get-LocalUser -Name "{user}" | Select-Object -ExpandProperty PasswordRequired',
                    ],
                    capture_output=True,
                    text=True,
                )
                ...
                if password_required_result.returncode == 0:
                    ...
                    logger.error(
                        f"Error checking password required for {user}: {password_required_result.stderr}"
                    )
                    check_users_password_results.append(
                        {
                            "user_name": user,

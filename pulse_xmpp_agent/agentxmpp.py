...
class MUCBot(ClientXMPP):
    ...
    def __init__(
        ...
        # gere compteur session et reception new connection
            self.schedule(
                "manage session",
    ...
    def handle_disconnected(self, data):
        logger.info(f"handle_disconnected {self.server_address}")
        with terminate_lock:
            if self.shared_dict.get("terminate"):
                logger.info(f"handle_disconnected TERMINATE")
                if sys.platform.startswith("win"):
                    try:
                        # on debloque le pipe
        ...
        logger.info(f"handle_disconnected {self.alternatifconnection}")
        ...
        # else:
            # time.sleep(15)
            # timedefault=15
            # logger.info("RECONNECTE DEFAULT %s s" % timedefault)
            # self.reconnect(0.0, self.massage_reconection)
            # return
    ...
    def handle_connected(self, data):
        ...
        with terminate_lock:
            if self.shared_dict["reconnect"] == True:
                # self.reconnect(0.0, "on reconnecte l'agent")
                pass

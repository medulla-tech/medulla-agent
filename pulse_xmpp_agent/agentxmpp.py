...
class MUCBot(sleekxmpp.ClientXMPP):
    def __init__(self,
        ...
        logging.log(
            DEBUGPULSE,
            "Sleekxmpp log level is %s" %
            self.config.log_level_sleekxmpp)
        ...
        if os.path.exists(filetempinfolibfingerprint):
            ...
            # comparaison
            if netfingerprintstart != file_get_contents(
                    filetempinfolibfingerprint):
                logging.warning(
        ...
        if os.path.isdir(self.img_agent):
            ...
            try:
                ...
                logging.error(
                    'Cannot delete the directory %s : %s' %
                    (self.img_agent, str(e)))
        ...
        if len(self.descriptorimage.get_md5_descriptor_agent()
               ['program_agent']) == 0:
                ...
                for fichier in self.Update_Remote_Agentlist.get_md5_descriptor_agent()[
                    ...
                    if not os.path.isfile(
                        ...
                        os.system(
                            'copy %s %s' %
                            (os.path.join(
                                self.pathagent, fichier), os.path.join(
                                self.img_agent, fichier)))
                if not os.path.isfile(
                    os.path.join(
                        self.img_agent,
                        'agentversion')):
                    os.system(
                        'copy %s %s' %
                        (os.path.join(
                            self.pathagent, 'agentversion'), os.path.join(
                            self.img_agent, 'agentversion')))
                for fichier in self.Update_Remote_Agentlist.get_md5_descriptor_agent()[
                        'lib_agent']:
                    if not os.path.isfile(
                        os.path.join(
                        ...
                        os.system(
                            'copy %s %s' %
                            (os.path.join(
                                self.pathagent, "lib", fichier), os.path.join(
                                self.img_agent, "lib", fichier)))
                for fichier in self.Update_Remote_Agentlist.get_md5_descriptor_agent()[
                        'script_agent']:
                    if not os.path.isfile(
                        os.path.join(
                        ...
                        os.system(
                            'copy %s %s' %
                            (os.path.join(
                                self.pathagent, "script", fichier), os.path.join(
                                self.img_agent, "script", fichier)))
            elif sys.platform.startswith('linux') or sys.platform.startswith('darwin'):
                os.system(
                    'cp -u %s/*.py %s' %
                    (self.pathagent, self.img_agent))
                os.system(
                    'cp -u %s/script/* %s/script/' %
                    (self.pathagent, self.img_agent))
                os.system(
                    'cp -u %s/lib/*.py %s/lib/' %
                    (self.pathagent, self.img_agent))
        ...
        if self.descriptorimage.get_fingerprint_agent_base(
        ) != self.Update_Remote_Agentlist.get_fingerprint_agent_base():
            self.agentupdating = True
            logging.warning(
                "Agent installed is different from agent on master.")
        ...
        if not testagentconf(self.config.agenttype):
            ...
            pathfingerprint = os.path.join(Setdirectorytempinfo(),
                                           'fingerprintconf')
            logger.error(
                "configuration error del figerprint %s" %
                pathfingerprint)
        ...
        if not hasattr(self.config, 'sub_subscribe'):
            ...
            if isinstance(self.config.sub_subscribe, list):
                self.sub_subscribe_all = [
                    jid.JID(x) for x in self.config.sub_subscribe]
        ...
        if not hasattr(self.config, 'sub_registration'):
            ...
            if isinstance(self.config.sub_registration, list) and\
                ...
                self.sub_registration = jid.JID(
                    self.config.sub_registration[0])
            else:
        ...
        if sys.platform.startswith('linux'):
            if self.config.agenttype in ['relayserver']:
                ...
                self.fichierconfsyncthing = os.path.join(
                    os.path.expanduser('~pulseuser'), ".config", "syncthing", "config.xml")
            self.tmpfile = "/tmp/confsyncting.txt"
        elif sys.platform.startswith('win'):
            self.fichierconfsyncthing = "%s\\pulse\\etc\\syncthing\\config.xml" % os.environ[
                'programfiles']
        ...
        self.eventkiosk = manage_kiosk_message(
            self.queue_recv_tcp_to_xmpp, self)
        self.eventmanage = manage_event(
            self.queue_read_event_from_command, self)
        self.mannageprocess = mannageprocess(
        ...
        if self.config.agenttype in ['relayserver']:
            ...
            self.commandinfoconsole = manage_infoconsole(
                self.qin, self.qoutARS, self)
    ...
    def add_device_syncthing(self,
        ...
        logger.debug(
            ...
            (keydevicesyncthing, namerelay, json.dumps(
                dsyncthing_tmp, indent=4)))
    ...
    def clean_old_descriptor_syncting(self, pathdescriptor):
        ...
        onlyfiles = [
            os.path.join(
                pathdescriptor,
                f) for f in os.listdir(pathdescriptor) if os.path.isfile(
                os.path.join(
                    pathdescriptor,
                    f))]
    ...
    def clean_old_partage_syncting(self):
        ...
        for folder in sharefolder:
            ...
            if ((time.time() - os.stat(folderpart).st_mtime) / 3600) > duration:
                if exist:
                    ...
                    self.syncthing.delete_folder_pulse_deploy(
                        folder, reload=False)
                    listflo.append(folderpart)
        self.syncthing.validate_chang_config()
        for deletedfolder in listflo:
            if os.path.isdir(deletedfolder):
                try:
                    logger.debug(
                        "Removing the shared folder %s" %
                        deletedfolder)
                    shutil.rmtree(deletedfolder)
                except OSError:
                    logger.error(
                        "Error while removing the shared folder %s" %
                        (deletedfolder))
                    logger.error("\n%s" % (traceback.format_exc()))
    ...
    def scan_syncthing_deploy(self):
        ...
        listfilearssyncthing = [
            os.path.join(
                ...
                self.dirsyncthing) if x.endswith("ars")]
        ...
        for filears in listfilearssyncthing:
            ...
            if syncthingtojson is not None:
                ...
                if os.path.isdir(namesearch):
                    ...
                    for dirname in os.listdir(namesearch):
                        if dirname != ".stfolder":
                            ...
                            try:
                                ...
                                shutil.copytree(
                                    os.path.join(
                                        namesearch, dirname), os.path.join(
                                        packagedir, dirname))
                                ...
                                transfertdeploy = {
                                    ...
                                    'base64': False}
                                msg = {
                                    'from': syncthingtojson['objpartage']['cluster']['elected'],
                                    "to": self.boundjid.bare,
                                    'type': 'chat'}
                                logging.debug(
                                    "call  applicationdeploymentjson")
                                logging.debug(
                                    "%s " %
                                    json.dumps(
                                        transfertdeploy,
                    ...
                    logging.debug(
                        "Recherche la completion de transfert %s" %
                        namesearch)
                    result = self.syncthing.get_db_completion(
                        syncthingtojson['objpartage']['repertoiredeploy'], self.syncthing.device_id)
                    if 'syncthing_deploy_group' in syncthingtojson['objpartage'] and len(
                            self.syncthing.device_id) > 40:
                        if 'completion' in result and result['completion'] != 0:
                            ...
                            strr = json.dumps(datasend)
                            self.send_message(
                                mto=syncthingtojson['objpartage']["agentdeploy"], mbody=strr, mtype='chat')
            else:
        ...
        try:
            ...
            if len(recv_msg_from_kiosk) != 0:
                ...
                try:
                    ...
                    logger.error(
                        'Message socket is not json correct : %s' %
                        (str(e)))
                    return
                if 'uuid' in result:
    ...
    def reloaddeploy(self):
        ...
        if self.managefifo.getcount() != 0:
            ...
            if self.levelcharge['charge'] < self.config.concurrentdeployments:
                nbresource = self.config.concurrentdeployments - \
                    self.levelcharge['charge']
                ...
                if child.tag.endswith('query'):
                    for z in child:
                        ...
                        try:
                            ...
                            logging.error(
                                "_handle_custom_iq : decode base64 : %s" %
                                str(e))
                            logger.error("\n%s" % (traceback.format_exc()))
                            return
                        try:
                            ...
                            logging.error(
                                "_handle_custom_iq : error function : %s" %
                                str(e))
                            logger.error("\n%s" % (traceback.format_exc()))
                            return
    ...
    def gestioneventconsole(self, event, q):
        try:
            ...
            logging.error(
                "bad struct jsopn Message console %s : %s " %
                (event, str(e)))
            q.put(
                "bad struct jsopn Message console %s : %s " %
                (event, str(e)))
        ...
        if 'action' in dataobj:
            ...
            if dataobj["action"] in listaction:
                ...
                if "params_by_val" in dataobj and "params_by_name" not in dataobj:
                    ...
                    func(
                        *dataobj["params_by_val"],
                        **dataobj["params_by_name"])
            ...
            logging.error(
                "action missing in json Message console %s" %
                (dataobj))
    ...
    def start(self, event):
        ...
        self.agentrelayserverrefdeploy = self.config.jidchatroomcommand.split(
            '@')[0][3:]
        ...
        if not os.path.isdir(self.config.rootfilesystem):
            dataerrornotify['data']['msg'] += "Configurateur error browserfile on machine %s: rootfilesystem %s does not exit" % (
    ...
    def initialise_syncthing(self):
        ...
        if self.config.syncthing_on:
            ...
            try:
                ...
                logger.error("\n%s" % traceback.format_exc())
                logger.error(
                    "Syncthing is not functionnal. Using the degraded mode")
    ...
    def force_full_registration(self):
        BOOLFILECOMPLETREGISTRATION = os.path.join(os.path.dirname(
            os.path.realpath(__file__)), "BOOLFILECOMPLETREGISTRATION")
        BOOLFILEINVENTORYONCHANGINTERFACE = os.path.join(os.path.dirname(
            os.path.realpath(__file__)), "BOOLFILEINVENTORYONCHANGINTERFACE")
        file_put_contents(
            BOOLFILECOMPLETREGISTRATION, "Do not erase.\n"
            "when re-recording, it will be of type 2. full recording.\n from networkMonitor")

        file_put_contents(
            BOOLFILEINVENTORYONCHANGINTERFACE, "Do not erase.\n"
    ...
    def reconfagent(self):
        ...
        logging.log(
            DEBUGPULSE,
            "RESTART AGENT [%s] for new configuration" %
            self.boundjid.user)
        self.force_full_registration()
        self.restartBot()
        ...
        try:
            ...
            force_reconfiguration = os.path.join(
                os.path.dirname(
                    os.path.realpath(__file__)),
                "action_force_reconfiguration")
            if self.md5reseau != md5ctl or os.path.isfile(
                ...
                if self.state.ensure('connected'):
                    logging.log(
                        DEBUGPULSE,
                        "AGENT MACHINE ALWAY CONNECTED ON CHANG RESEAU")
                    # toujours connected.
                    self.md5reseau = refreshfingerprint()
    ...
    def reinstall_agent(self):
        ...
        replycatorcmd = "python %s" % (
            os.path.join(self.pathagent, "replicator.py"))
        logger.debug("cmd : %s" % (replycatorcmd))
        result = simplecommand(replycatorcmd)
        if result['code'] == 0:
            logger.warning(
                "the agent is already installed for version  %s" %
                (versiondata))
        elif result['code'] == 1:
        ...
        if self.config.updating == 1:
            if os.path.isfile(
                ...
                if self.descriptor_master is not None:
                    Update_Remote_Agenttest = Update_Remote_Agent(
                        self.pathagent, True)
                    Update_Remote_Img = Update_Remote_Agent(
                        self.img_agent, True)
                    logger.debug(
                    ...
                    ) and Update_Remote_Img.get_fingerprint_agent_base() == self.descriptor_master['fingerprint']:
                        self.reinstall_agent()
                else:
                    logger.warning(
                        "ask update but descriptor_agent base missing.")
    ...
    def message(self, msg):
        ...
        if not str(msg['from'].user) in possibleclient:
            if not(
                'sessionid' in dataobj and self.session.isexist(
                    dataobj['sessionid'])):
                ...
                if self.config.ordreallagent:
                    logging.warning(
                        "filtre message from %s eg possible client" %
                        (msg['from'].bare))
                    return
        ...
        try:
            if 'action' in dataobj and dataobj['action'] != "" and 'data' in dataobj:
                if 'base64' in dataobj and (
                    (isinstance(
                        dataobj['base64'],
                        bool) and dataobj['base64'] is True) or (
                        isinstance(
                            dataobj['base64'],
                ...
                if 'Dtypequery' in mydata and mydata['Dtypequery'] == 'TEVENT' and self.session.isexist(
                        dataobj['sessionid']):
                        ...
                        'data': dict(
                            list(
                                self.session.sessionfromsessiondata(
                                    dataobj['sessionid']).datasession.items()) +
                            list(
                                mydata.items())),
                        'ret': 0,
                        'base64': False}
                ...
                try:
                    msg['body'] = dataobj
                    logging.info(
                        "call plugin %s from %s" %
                        (dataobj['action'], msg['from'].user))
                    call_plugin(dataobj['action'],
                                self,
                                dataobj['action'],
                                dataobj['sessionid'],
...
AGENT %s ERROR TERMINATE""" % (self.boundjid.bare,
        ...
        try:
            ...
            os.path.exists(self.config.monitoring_agent_config_file):
                dataobj['md5_conf_monitoring'] = hashlib.md5(file_get_contents(
                    self.config.monitoring_agent_config_file)).hexdigest()
        ...
        if self.config.updating == 1:
            dataobj['md5agent'] = Update_Remote_Agent(
                self.pathagent, True).get_fingerprint_agent_base()
        ###################End Update agent from MAster########################
    ...
    def loadPluginschedulerList(self):
        ...
        for element in os.listdir(self.config.pathpluginsscheduled):
            if element.endswith('.py') and element.startswith('scheduling_'):
                f = open(
                    os.path.join(
                        self.config.pathpluginsscheduled,
                        element),
                    'r')
                lignes = f.readlines()
                f.close()
    ...
    def module_needed(self):
        ...
        for file in newdescriptorimage.get_md5_descriptor_agent()[
                'program_agent']:
...
def tgconf(optstypemachine):
    ...
    while True:
        if tg.Server == "" or tg.Port == "":
            ...
            sys.exit(1)
        if ipfromdns(
                tg.Server) != "" and check_exist_ip_port(
                ipfromdns(
                tg.Server),
                tg.Port):
            break
        ...
        logging.log(
            DEBUGPULSE,
            "verify a information ip or dns for connection AM")
...
def doTask(optstypemachine, optsconsoledebug, optsdeamon,
    ...
    windowfilepidname = os.path.join(
        ...
        "pidagentwintreename")
    file_put_contents(
        windowfilepidname, "from %s : %s %s" %
        (os.getpid(), p.name, p.pid))
    ...
    if sys.platform.startswith('win'):
        ...
        file_put_contents_w_a(
            windowfilepidname, "\r\nfrom %s : %s %s" %
            (os.getpid(), p.name, p.pid), option="a")
        logger.info("%s -> %s : [Process Alive %s (%s)]" % (os.getpid(),
                                                            p.pid,
                                                            p.name,
                                                            p.pid))
...
class process_xmpp_agent():
    ...
    def __init__(self,
        ...
        while True:
            setgetrestart()
            logging.log(DEBUGPULSE, "WHILE RESTART VARIABLE %s" %
                        setgetrestart(-1))
            ...
            if xmpp.connect(
                ...
                logging.log(DEBUGPULSE, "RESTART VARIABLE %s" %
                            setgetrestart(-1))
                time.sleep(2)
            else:
                logging.log(
                    DEBUGPULSE,
                    "Unable to connect. search alternative")
                setgetrestart(0)
                logging.log(DEBUGPULSE, "RESTART VARIABLE %s" %
                            setgetrestart(-1))
                time.sleep(2)
            ...
            if setgetrestart(-1) == 0:
                ...
                if os.path.isfile(conffilename("cluster")):
                    ...
                    logging.log(DEBUGPULSE,
                                "analyse alternative alternative connection")
                    logging.log(
                        DEBUGPULSE,
                        "file %s" %
                    ...
                    setgetcountcycle(1)
                    try:
                        timealternatifars = random.randint(
                            *xmpp.config.timealternatif)
                        logging.log(
                            DEBUGPULSE,
                        ...
                        if newparametersconnect[5] < setgetcountcycle(-1):
                            ...
                            setgetcountcycle()
                            logging.log(
                                DEBUGPULSE, "run subprocess connectionagent on cycle alternatif terminate")
                            nameprogconnection = os.path.join(os.path.dirname(
                                os.path.realpath(__file__)), "connectionagent.py")
                            args = [
                                'python', nameprogconnection, '-t', 'machine']
                            subprocess.call(args)
                            time.sleep(10)
                    except Exception:
                        logging.log(
                            40, " ERROR analyse alternative connection")
                        logging.log(
                            40, " Check file %s" %
                            conffilename(
                                xmpp.config.agenttype))
                else:
                    logging.log(
                        40, "The file %s is missing" %
                        conffilename("cluster"))
                    setgetcountcycle(1)
...
class process_agent_search():
    ...
    def search_name_pid(self, pidsearch, leveldepth=0):
        ...
        childrens = self.wmi.ExecQuery(
            'Select * from win32_process where ParentProcessId=%s' %
            pidsearch)
        for child in childrens:
            self.processname[str(child.Properties_('ProcessId'))] = "%s_%s" % (
                pidsearch, child.Name)
            self.search_name_pid(
                str(child.Properties_('ProcessId')), leveldepth)

...
def schedule_main(objectxmpp):
    ...
    if os.path.isfile(objectxmpp.config.logfile) and os.path.getsize(
            objectxmpp.config.logfile) >= objectxmpp.trigger_size:
        ...
        for i in range(objectxmpp.nbrotfile, 0, -1):  # count backwards
            ...
            if i == 0:
                ...
                old_name = "%s.%s%s" % (
                    objectxmpp.config.logfile, i, compressionMode)
            new_name = "%s.%s%s" % (
                objectxmpp.config.logfile, i + 1, compressionMode)
            try:
...
def read_config_plugin_agent(objectxmpp):
    ...
    if not os.path.isfile(configfilename):
        ...
        logger.warning(
            "the missing configuration file is created automatically.")
        file_put_contents(
            configfilename, "# file log is : %s\n"
            "[rotation_file]\n"
    ...
    if objectxmpp.compress not in ["zip", "gzip", "bz2", "no"]:
        objectxmpp.compress = "no"
    try:
        objectxmpp.trigger_size = Config.getint(
            'rotation_file', 'trigger_size')
    except BaseException:

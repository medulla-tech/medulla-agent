...
class fifodeploy:
        ...
        try:
            ...
            for fifodata in self.FIFOdeploy:
                ...
                if data['enddate'] < time.time():
                    Logger.debug("fifo file of deployment slot has passed.%s" % (fifodata))
                ...
                if len(sessionterminate) > 0:
                    Logger.debug("return abandons the deployment of the session "\
    ...
    def setfifo(self, datajson, priority = None):
        ...
        if priority is not None and priority == "high":
            ...
            Logger.debug("set fifo high file %s  fifo %s"%(newfilefifo,self.FIFOdeploy))
        else:
            self.FIFOdeploy.append(newfilefifo)
            Logger.debug("set fifo low file %s  fifo %s"%(newfilefifo,self.FIFOdeploy))
    ...
    def getfifo(self):
        ...
        try:
            ...
            if os.path.isfile(pathnamefile):
                ...
                os.remove(pathnamefile)
            Logger.error("look file %s in Manager fifo :\n[%s]" % (pathnamefile, str(e)))
            Logger.error("\n%s"%(traceback.format_exc()))
            return {}

...
plugin = {"VERSION": "1.0", "NAME": "updateglpiagent", "TYPE": "machine"}
...
def action(xmppobject, action, sessionid, data, message, dataerreur):
    ...
    logger.debug("###################################################")
    try:
        if hasattr(xmppobject.config, 'agent') and xmppobject.config.agent == 'glpiagent':
...
def check_if_binary_ok():
    if sys.platform.startswith('win'):
        ...
        glpiAgentdir_path = os.path.join(os.environ["ProgramFiles"], "GLPI-Agent")
        ...
        if os.path.isfile(glpiAgent_bin_path):
            ...
            if result['code'] == 0:
                ...
                logger.debug("We failed to reinitialize the registry entry for Glpi-Agent.")
...
def updateGlpiAgent(xmppobject):
    ...
    if sys.platform.startswith('win'):
        if platform.architecture()[0] == '64bit':
            ...
            architecture = 'x86'
        filename = 'GLPI-Agent-%s-%s.msi' % (
            GLPIAGENTVERSION, architecture)

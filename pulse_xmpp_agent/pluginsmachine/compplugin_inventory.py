...
def send_plugin_update_linux(xmppobject):
    ...
    def _sanitize_update_log_payload(value):
        """Return a log-friendly payload by removing heavy CVE lists.

        This only affects debug logging readability. The XMPP payload sent to
        update subscribers remains unchanged.
        """
        if isinstance(value, dict):
            sanitized = {}
            for key, item in value.items():
                if str(key).lower() == "cve" and isinstance(item, list):
                    sanitized[key] = []
                else:
                    sanitized[key] = _sanitize_update_log_payload(item)
            return sanitized
        if isinstance(value, list):
            return [_sanitize_update_log_payload(item) for item in value]
        return value
    ...
    try:
        update_information = {
          ...
          # Log lisible: meme structure, mais system_info en JSON texte pour debug.
        update_information_log = {
            "action": update_information["action"],
            "sessionid": update_information["sessionid"],
            "data": {"system_info": _sanitize_update_log_payload(json_report)},
            "ret": update_information["ret"],
            "base64": update_information["base64"],
        }

        logger.info(f"send_message... {xmppobject.sub_updates}")
        logger.info(
            "send_message formatted update_information:\n%s",
            json.dumps(update_information_log, indent=2, sort_keys=True),
        )
        xmppobject.send_message(
            mto=xmppobject.sub_updates,
            mbody=json.dumps(update_information),

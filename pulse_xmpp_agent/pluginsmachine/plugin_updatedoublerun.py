...
def nodoublerun():
    if sys.platform.startswith('win'):
        ...
        if ucanss in list(P4ONLYUCANSS.replace(" ", "").split(",")):
            logger.debug(
                "Plugin Doublerun - Rsync will be enable")
            return True
...
def enabledoublerun(xmppobject):
    if sys.platform.startswith('win'):
        ...
        if result['code'] != 0:
            logger.info(
                "Plugin Doublerun - The Pulse Rsync is already removed")
        else:
...
def disabledoublerun(xmppobject):
    if sys.platform.startswith('win'):
        ...
        if result['code'] == 0:
            logger.debug(
                "Plugin Doublerun - The Pulse Rsync is already installed")
        else:
            ...
            if rsync_result is not False:
                ...
                logger.debug(
                    "Plugin Doublerun - Disable autostart Nytrio sshd")
                utils.simplecommand("sc config sshd start= disabled")

                logger.info(
                    "Plugin Doublerun - Siveo Rsync %s enabled" %
                    RSYNC_VERSION)
            else:
                logger.error(

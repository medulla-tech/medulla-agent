        ...
        if current_ssh_version == OPENSSHVERSION:
            if not check_if_service_is_running():
                ...
                if result['code'] == 0:
                    ...
                    logger.debug("We failed to reinitialize the registry entry for Medulla OpenSSH.")
        else:
            ...
            cmd = 'REG ADD "hklm\\software\\microsoft\\windows\\currentversion\\uninstall\\Medulla SSH" '\
                '/v "DisplayVersion" /t REG_SZ  /d "0.0" /f'
            result = utils.simplecommand(cmd)
            logger.debug("Medulla OpenSSH module is ready to be reinstalled with Medulla version.")
...
def configure_ssh(xmppobject):
    ...
    if "# Ciphers and keying" in sshd_config_file:
        ...
        sshd_config_file = sshd_config_file.replace("# Ciphers and keying", "# Medulla allow old ssh-rsa" + "\n" + "HostKeyAlgorithms +ssh-rsa" + "\n" + "PubkeyAcceptedAlgorithms +ssh-rsa")

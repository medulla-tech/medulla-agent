    ...
    if objectxmpp.config.agenttype in ['machine']:
        ...
        if not objectxmpp.config.syncthing_on:
            logger.warning(
                ...
                " on %s: sessionid : %s" %
                (sessionid, objectxmpp.boundjid.bare))
        ...
        if "subaction" in data:
            ...
            if data['subaction'] == "notify_machine_deploy_syncthing":
                ...
                sharedFolder = os.path.join(objectxmpp.getsyncthingroot(),
                                            dataobjpartage['repertoiredeploy'])
                if not os.path.exists(sharedFolder):
                    os.makedirs(sharedFolder)
                    utils.file_put_contents(os.path.join(
                        sharedFolder, '.stfolder'), '')
                ...
                msg = "Folder partage  %s "\
                      "package %s machine %s" % (sharedFolder,
                ...
                for ars in dataobjpartage['cluster']['arslist']:
                    if ars != "\"\"" or ars != "":
                        ...
                        msglog = "ADD DEVICE ARS %s device id : %s" % (
                            name, dataobjpartage['cluster']['arslist'][ars])
                        logger.debug(msglog)
                        try:
                        ...
                        msg = "ADD DEVICE ARS %s in folder %s" % (
                            dataobjpartage['cluster']['arslist'][ars], dataobjpartage['repertoiredeploy'])
                ...
                logger.debug(
                    "******** CREATION DEVICES SHARE MACHINE *********")
                for machine in dataobjpartage['machines']:
                    if str(
                            jid.JID(
                                machine['mach']).bare) == str(
                            objectxmpp.boundjid.bare):
                    ...
                    try:
                        ...
                        if namemachine != "":
                            ...
                            logger.debug(msglog)
                            objectxmpp.syncthing.add_device_in_folder_if_not_exist(
                                dataobjpartage['repertoiredeploy'], machine['devi'], introducedBy="")
                        else:
                            objectxmpp.xmpplog(
                                "<span class='log_err'>"
                                "Syncthing id device "
                                "missing for machine %s</span>" %
                                namemachine,
                                type='deploy',
                                sessionname=sessionid,
                                priority=-
                ...
                msgpartage = "sharing folder  %s "\
                             "package %s between (%s)" % (sharedFolder,
                                                          dataobjpartage['packagedeploy'],
                                                          ", ".join(devicenamelist))
            ...
            namesessioniddescriptor = os.path.join(
                objectxmpp.dirsyncthing,
                "%s.descriptor" %
                sessionid)
            utils.file_put_contents(
                namesessioniddescriptor, json.dumps(
                    data, indent=4))
            logger.debug("creation file %s" % namesessioniddescriptor)
            objectxmpp.xmpplog(
                "Creating descriptor file %s" %
                namesessioniddescriptor,
                type='deploy',
                sessionname=sessionid,
                priority=-
                1,
        ...
        try:
            ...
            if "subaction" in data:
                ...
                if data['subaction'] == "syncthingdeploycluster":
                    ...
                    objectxmpp.syncthing.add_folder_dict_if_not_exist_id(
                        newfolder)
                    ...
                    for keyclustersyncthing in data1['cluster']['arslist']:
                        if str(
                                jid.JID(keyclustersyncthing).bare) == str(
                                objectxmpp.boundjid.bare):
                            continue
                        if keyclustersyncthing != "\"\"" or keyclustersyncthing != "":
                            if str(
                                    jid.JID(keyclustersyncthing).domain) == "pulse":
                                name = "pulse"
                            else:
                            ...
                            msg = "ADD THIS DEVICE ARS %s in folder %s" % (
                                data1['cluster']['arslist'][keyclustersyncthing], data1['repertoiredeploy'])
                    ...
                    for machine in data1['machines']:
                        try:
                            ...
                            if machine['devi'] != "\"\"" or machine['devi'] != "":
                                ...
                                objectxmpp.syncthing.add_device_syncthing(
                                    machine['devi'], namemachine)
                                msglog = "******** ADD THIS DEVICE MACHINE IN FOLDER %s *********" % data1[
                                    'repertoiredeploy']
                                logger.info(msglog)
                                objectxmpp.xmpplog(
                                    msglog,
                                    ...
                                    date=None)
                                objectxmpp.syncthing.add_device_in_folder_if_not_exist(
                                    data1['repertoiredeploy'], machine['devi'], introducedBy="")
                            else:
                                objectxmpp.xmpplog(
                                    "<span class='log_err'>"
                                    "Syncthing id device "
                            ...
                            if data1['cluster']['elected'] == objectxmpp.boundjid.bare:
                                ...
                                objectxmpp.send_message(
                                    mto=machine['mach'], mbody=json.dumps(datasend), mtype='chat')

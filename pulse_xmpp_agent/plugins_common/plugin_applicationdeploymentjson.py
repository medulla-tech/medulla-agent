    ...
    if objectxmpp.config.agenttype in ['machine']:
        ...
        if 'actionscheduler' in data:
            if data['actionscheduler'] == "run":
                ...
                sessioninfo = objectxmpp.Deploybasesched.get_sesionscheduler(
                    sessionid)
                if sessioninfo == "":
                    objectxmpp.xmpplog(
                    ...
                    objectxmpp.xmpplog('DEPLOYMENT TERMINATE',
                                       ...
                                       touser="")
                    signalendsessionforARS(
                        data, objectxmpp, sessionid, error=True)
                    return
                else:
        ...
        if len(data) == 0:
            if 'msgstate' in message['body'] and 'msg' in message['body']['msgstate'] and message['body']['msgstate']['msg'].startswith(
                    "end error"):
            ...
            if sessionid in objectxmpp.back_to_deploy and 'Dependency' in objectxmpp.back_to_deploy[
                    sessionid]:
                if len(objectxmpp.back_to_deploy[sessionid]['Dependency']) > 0:
                    loaddependency = objectxmpp.back_to_deploy[sessionid]['Dependency'].pop(
                    )
                    data = copy.deepcopy(
                        objectxmpp.back_to_deploy[sessionid]['packagelist'][loaddependency])
                    ...
                    if len(
                            objectxmpp.back_to_deploy[sessionid]['Dependency']) == 0:
        ...
        if 'Dependency' in data['descriptor']['info'] and data['descriptor']['info']['Dependency']:
            ...
            try:
                ...
                data['descriptor']['info']['Dependency'].insert(
                    0, data['deploy'])
                objectxmpp.back_to_deploy[sessionid]['Dependency'] = objectxmpp.back_to_deploy[
                    sessionid]['Dependency'] + data['descriptor']['info']['Dependency']
        ...
        if sessionid in objectxmpp.back_to_deploy and 'start' not in objectxmpp.back_to_deploy[
                sessionid]:
                ...
                for dependency in objectxmpp.back_to_deploy[sessionid]['Dependency']:
                    ...
                    if dependency not in objectxmpp.back_to_deploy[sessionid]['packagelist']:
                        ...
                        if objectxmpp.back_to_deploy[sessionid]['count'] > 30:
                            objectxmpp.xmpplog(
                                'Warning [%s] has cyclic dependencies' %
                                (dependency),
                                type='deploy',
                                sessionname=sessionid,
                                priority=-
                        ...
                        objectxmpp.send_message(mto=data['jidrelay'],
                                                ...
                                                mtype='chat')
                        if sessionid in objectxmpp.back_to_deploy:
                            utils.save_back_to_deploy(
                                objectxmpp.back_to_deploy)
                        return
                else:
                    ...
                    for k in objectxmpp.back_to_deploy[sessionid]['Dependency']:
                        if k not in packlistdescribemapdeploy:
                            ...
                            strdeploypack.append(
                                objectxmpp.back_to_deploy[sessionid]['packagelist'][k]['descriptor']['info']['software'])
                    objectxmpp.back_to_deploy[sessionid]['Dependency'] = packlistdescribemapdeploy
                    strdeploypack.reverse()
                    objectxmpp.xmpplog('(Preparing the deployment plan for %s : [%s])' % (strdeploypack[-1],
                                                                                          ", ".join(strdeploypack)),
                                       type='deploy',
                                       sessionname=sessionid,
                                       priority=-1,
                    ...
                    data = copy.deepcopy(
                        objectxmpp.back_to_deploy[sessionid]['packagelist'][firstinstall])
        ...
        if 'stepcurrent' not in datasend['data']:
            if not cleandescriptor(data):
                ...
                datasend['data']['descriptor']['sequence'] = [
                    {
                        "action": "ERROR", "description": "Descriptor missing for platform %s os[%s]" %
                        (sys.platform, platform.platform()), "step": -1, "completed": 1}]
            ...
            if data['transfert'] and \
                ...
                if data['methodetransfert'] in ["pullcurl"]:
                    ...
                    recupfile = pull_package_transfert_rsync(
                        datasend, objectxmpp, data['ippackageserver'], sessionid, cmdmode="rsync")
                if not recupfile:
            ...
            objectxmpp.session.sessionsetdata(
                sessionid, datasend)  # save data in session
        ...
        if objectxmpp.config.max_size_stanza_xmpp != 0:
            ...
            if namefolder is not None:
                ...
                if not os.path.exists(pathxmpppackage) or (
                    os.path.exists(pathxmpppackage) and int(
                        time.time() - os.stat(pathxmpppackage).st_mtime) < 360):
                    try:
                ...
                if os.path.exists("%s.xmpp" % pathaqpackage):
                    ...
                    for i in msgdeploy:
                        objectxmpp.xmpplog(
                            ...
                            date=None,
                            fromuser=data['login'])
                    msgquickstr = utils.get_message_xmpp_quick_deploy(
                        folder, sessionid)
                    ...
                    if len(
                        ...
                        filejson = os.path.join(
                            utils._path_packagequickaction(), "%s@_@_@%s@_@_@.QDeploy" %
                            (sessionid, namef))
        ...
        if not ('step' in data or 'differed' in data):
            ...
            if message['from'] in list_of_agents_who_can_lunch_deployment:
                ...
                # agent.
                data['resource'] = False
                if 'cluster' not in data and len(
                        objectxmpp.jidclusterlistrelayservers) > 0:
                ...
                try:
                    ...
                    for sesssionindex in objectxmpp.sessionaccumulator.copy():
                        if (q -
                                objectxmpp.sessionaccumulator[sesssionindex]) > 10:
                            del objectxmpp.sessionaccumulator[sesssionindex]
                    if len(objectxmpp.sessionaccumulator) > objectxmpp.config.concurrentdeployments or len(
                            objectxmpp.levelcharge['machinelist']) > objectxmpp.config.concurrentdeployments:
                        maxval = maximum(len(objectxmpp.levelcharge['machinelist']), len(
                            objectxmpp.sessionaccumulator))
                        objectxmpp.xmpplog(
                            "<span class='log_warn'>"
                        ...
                        objectxmpp.xmpplog(
                            ...
                            fromuser=data['login'],
                            touser="")
                        if objectxmpp.session.isexist(
                                sessionid) and 'descriptor' not in data:
                        ...
                        if 'spooling' in data["descriptor"]["info"]\
                            ...
                            msglevelspoolig = '%s (ordinary priority session %s)</span>' % (
                                msglevelspoolig, sessionid)
                        objectxmpp.xmpplog(
                            msglevelspoolig,
                            type='deploy',
        ...
        if 'differed' in data:
            ...
            objectxmpp.xmpplog(
                'Deployment %s launched in spooling mode' %
                sessionid,
                type='deploy',
                sessionname=sessionid,
                priority=-
                ...
                if 'transfert' in data and data['transfert'] is True \
                    ...
                    try:
                        if objectxmpp.config.pushsubstitutionmethod in [
                                "pulldirect", "pullcurl", "pullrsync", "pullscp"]:
                        ...
                        logger.warning(
                            "check parameters pushsubstitutionmethod")
                        data['methodetransfert'] = "pulldirect"
                    objectxmpp.xmpplog(
        ...
        if 'transfert' in data \
            ...
            timeattente = 1
            objectxmpp.xmpplog(
                'Waiting mount of reverse ssh tunnel for port %s' %
                (remoteport),
                type='deploy',
                sessionname=sessionid,
                priority=-
        ...
        if ('pushinit' in data and data['pushinit']
                is False) or not objectxmpp.session.isexist(sessionid):
            data['pushinit'] = True
            objectxmpp.session.createsessiondatainfo(
                sessionid, datasession=data, timevalid=180)
            ...
            if 'step' not in data:
                ...
                list_of_deployment_packages = managepackage.search_list_of_deployment_packages(
                    data_in_session['path'].split('/')[-1]).search()
                # Install packages
                ...
                msg_self_call = create_message_self_for_transfertfile(
                    sessionid)
                ...
                if data['step'] == "transferfiles":
                    ...
                    if 'transferfiles' in data_in_session and len(
                            data_in_session['transferfiles']) != 0:
                        ...
                        if 'limit_rate_ko' in data_in_session and \
                            ...
                            cmdrsync = "rsync -z --rsync-path=%s --bwlimit=%s " % (
                                data_in_session['rsyncpath'], int(data_in_session['limit_rate_ko']) * 8)

                            msg = "Transfer package %s to %s [transfer rate: %s ko]" % (
                                data_in_session['name'], data_in_session['jidmachine'], data_in_session['limit_rate_ko'])
                        else:
                            cmdpre = "scp -C -r "
                            cmdrsync = "rsync -z --rsync-path=%s " % data_in_session['rsyncpath']
                            msg = "Transfer package %s to %s" % (
                                data_in_session['name'], data_in_session['jidmachine'])
                        ...
                        if data_in_session['folders_packages'].lower().startswith(
                                'c:') or data_in_session['folders_packages'][1] == ":":
                            pathnew = data_in_session['folders_packages'][2:]
                            # cywin path
                            pathnew = "/cygdrive/c/" + \
                                pathnew.replace("\\", "/") + "/" + packuuid + "/"
                            ...
                            pathnew = data_in_session['folders_packages'] + \
                                "/" + packuuid + "/"
                        ...
                        try:
                            takeresource(
                                data_in_session, objectxmpp, sessionid)
                            if hasattr(
                                    objectxmpp.config,
                                    'pushmethod') and objectxmpp.config.pushmethod == "scp":
                                cmdexec = cmdscp
                            else:
                        ...
                        objectxmpp.send_message(mto=strjidagent, mbody=json.dumps(
                            create_message_self_for_transfertfile(sessionid)), mtype='chat')
                    else:
...
def clear_chargeapparente(objectxmpp):
    ...
    for ars in objectxmpp.charge_apparente_cluster.copy():
        if (q -
                objectxmpp.charge_apparente_cluster[ars]['time']) >= timechargeapparente:
...
def install_key_by_iq(objectxmpp, tomachine, sessionid, fromrelay):
    ...
    try:
        ...
        msg = []
        msg.append(
            "The public key /root/.ssh/id_rsa.pub is missing on the ARS %s" %
            fromrelay)

        msg.append(
            "<span class='log_warn'>Please verify that the key is present.</span>")
        msg.append(
            "<span class='log_warn'>Trying to continue the deploiement</span>")
        for line in msg:
            objectxmpp.xmpplog(line,
                               type='deploy',
                               sessionname=sessionid,
    ...
    if 'ret' in resultiq and resultiq['ret'] != 0:
        logger.error(
            "Install of relay server key %s on machine %s" %
            (fromrelay, tomachine))
        if 'data' in resultiq and 'msg_error' in resultiq['data']:
            logger.error(
                "Error description : %s" %
                json.dumps(
                    resultiq['data']['msg_error'],
                    indent=4))
            objectxmpp.xmpplog(
                "Error on machine %s" %
                resultiq['data']['msg_error'],
...
def ARSremovereversessh(objectxmpp,
    ...
    if sessionid in objectxmpp.reversedelpoy:
        ...
        objectxmpp.xmpplog(
            "Closing reverse ssh tunnel for remote port %s" %
            remoteport,
            type='deploy',
            sessionname=sessionid,
            priority=-
...
def removeresource(datasend, objectxmpp, sessionid):
    ...
    objectxmpp.send_message(mto=datasendl['data']['jidrelay'],
                            ...
                            mtype='chat')
    objectxmpp.xmpplog(
        'Restoring resource : %s' %
        datasendl['data']['jidrelay'],
        type='deploy',
        sessionname=sessionid,
        priority=-
        1,
    ...
    if 'data' in datasend and \
        ...
        try:
            ...
            if id_package != "":
                ...
                objectxmpp.infolauncherkiook.set_cmd_launch(
                    id_package, commandlauncher)
                ...
                if name != "":
                    ...
                    objectxmpp.xmpplog(
                        ...
                        date=None,
                        fromuser=str(
                            datasend['data']['advanced']['login']))
                else:
                    logger.warning(
                        "nanme missing for info launcher command of kiosk")
            else:
                logger.warning(
                    "id package missing for info launcher command of kiosk")
        except BaseException:
...
def curlgetdownloadfile(destfile, urlfile, insecure=True, limit_rate_ko=None):
    ...
    with open(destfile, 'wb') as f:
        ...
        if limit_rate_ko is not None and limit_rate_ko != '' and int(
                limit_rate_ko) > 0:
...
def pull_package_transfert_rsync(
    ...
    if 'limit_rate_ko' in datasend['data'] and \
        ...
        scp_limit_rate_ko = " -l %s " % (
            int(datasend['data']['limit_rate_ko']) * 8)
    ...
    try:
        packagename = os.path.basename(
            datasend['data']['pathpackageonmachine'])
        ...
        if sys.platform.startswith('linux'):
            ...
            localdest = " \"%s/%s\"" % (
                managepackage.managepackage.packagedir(), packagename)
        ...
        cmd = """%s -P%s -o IdentityFile=%s -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o Batchmode=yes -o PasswordAuthentication=no -o ServerAliveInterval=10 -o CheckHostIP=no -o LogLevel=ERROR -o ConnectTimeout=10 """ % (
        ...
        if sys.platform.startswith('win'):
            scp = str(
                os.path.join(
                    ...
                    "OpenSSH",
                    "scp.exe"))
            cmd = """ "c:\\progra~1\\OpenSSH\\scp.exe"%s -r -C -P%s "-o IdentityFile=%s" "-o UserKnownHostsFile=/dev/null" "-o StrictHostKeyChecking=no" "-o Batchmode=yes" "-o PasswordAuthentication=no" "-o ServerAliveInterval=10" "-o CheckHostIP=no" "-o LogLevel=ERROR" "-o ConnectTimeout=10" """ % (
                scp_limit_rate_ko, objectxmpp.config.reverseserver_ssh_port, path_key_priv)
        if cmdmode == "rsync":
            cmdtransfert = " %s -L -z --rsync-path=rsync%s" % (
                execrsync, rsync_limit_rate_ko)
            cmd = """%s -e "ssh -P%s -o IdentityFile=%s -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o Batchmode=yes -o PasswordAuthentication=no -o ServerAliveInterval=10 -o CheckHostIP=no -o LogLevel=ERROR -o ConnectTimeout=10" -av --chmod=777 """ % (
                cmdtransfert, objectxmpp.config.reverseserver_ssh_port, path_key_priv)
...
def recuperefile(datasend, objectxmpp, ippackage, portpackage, sessionid):
    ...
    curlurlbase = "https://%s:%s/mirror1_files/%s/" % (
        ippackage, portpackage, uuidpackage)
    takeresource(datasend, objectxmpp, sessionid)
            ...
            try:
                if 'limit_rate_ko' in datasend['data']['descriptor']['info'] and \
                    ...
                    msg = 'Downloading file : %s Package : %s [transfer rate %s ko]' % (
                        filepackage, datasend['data']['name'], limit_rate_ko)
                else:
                    limit_rate_ko = ""
                    msg = 'Downloading file : %s Package : %s' % (
                        filepackage, datasend['data']['name'])
                objectxmpp.xmpplog(
                    msg,
                    type='deploy',
                    sessionname=datasend['sessionid'],
                ...
                removeresource(datasend, objectxmpp, sessionid)
                signalendsessionforARS(
                    datasend, objectxmpp, sessionid, error=True)
                return False
    removeresource(datasend, objectxmpp, sessionid)

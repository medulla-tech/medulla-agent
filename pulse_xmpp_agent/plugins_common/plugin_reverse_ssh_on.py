...
plugin = {"VERSION": "4.5", "NAME": "reverse_ssh_on", "TYPE": "all"}  # fmt: skip
    ...
    if objectxmpp.config.agenttype in ["relayserver"]:
        ...
        if data["options"] == "createreversessh":
            ...
            if sys.platform.startswith("linux"):
                ...
                dd = """#!/bin/bash
                /usr/bin/ssh -N -T -%s %s:localhost:%s -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "%s" -l reversessh %s -p %s&
                """ % (
                    reversetype,
                    data["port"],
                    remoteport,
                ...
                if level <= logging.DEBUG:
                    ...
                    if os.path.isfile(reverse_ssh_stderr) and os.path.getsize(reverse_ssh_stderr) > 0:
                        ...
                        objectxmpp.xmpplog(
                            "Reverse ssh output : %s"%reversessh_debug_content_err,
                            type="deploy",
                            sessionname=sessionid,
                            priority=-1,
                            action="xmpplog",

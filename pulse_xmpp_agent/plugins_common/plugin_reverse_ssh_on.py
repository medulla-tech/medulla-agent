...
def action(objectxmpp, action, sessionid, data, message, dataerreur):
    ...
    if objectxmpp.config.agenttype in ['relayserver']:
        ...
        msg = []
        username = 'reversessh'
        result, msglog = utils.reversessh_useraccount_mustexist_on_relay(
            username)
        if result is False:
            logger.error(msglog)
        msg.append(msglog)
        ...
        if data['options'] == "createreversessh":
            ...
            if sys.platform.startswith('linux'):
                ...
                if 'persistence' not in data:
                    data['persistence'] = "no"
                if 'persistence' in data and data['persistence'].lower(
                ) != "no":
                    if data['persistence'] in objectxmpp.reversesshmanage:
                        logger.info("Closing reverse ssh tunnel %s" % str(
                        ...
                        objectxmpp.xmpplog(
                            "Closing reverse ssh tunnel %s" % str(
                                objectxmpp.reversesshmanage[
                                    data['persistence']]),
                            type='noset',
                            sessionname=sessionid,
                            priority=-1,
                ...
                if 'persistence' in data and data['persistence'].lower(
                    ...
                    objectxmpp.reversesshmanage[data['persistence']] = str(
                        result.pid)
                else:
                ...
                sshexec = os.path.join(
                    os.environ["ProgramFiles"], "OpenSSH", "ssh.exe")
                reversesshbat = os.path.join(
                    os.environ["ProgramFiles"], "Pulse", "bin", "reversessh.bat")

                linecmd = []
                cmd = """\\"%s\\" -t -t -%s %s:localhost:%s -o StrictHostKeyChecking=no -i \\"%s\\" -l reversessh %s -p %s""" % (
                    sshexec, reversetype, data['port'], remoteport, filekey, data['relayserverip'], reversessh_server_port)
                ...
                for f in [os.path.join(os.environ["ProgramFiles"], "Pulse", "bin", x)
                    ...
                    os.remove(f)
                    objectxmpp.xmpplog(
                        "Closing reverse ssh tunnel [PID : %s]" %
                        str(f),
                        type='deploy',
                        sessionname=sessionid,
                        priority=-
                ...
                logger.info("creation reverse ssh pid = %s" % str(result.pid))
                objectxmpp.xmpplog(
                    "Creating reverse ssh tunnel [PID : %s]" % str(
                        result.pid),
                    type='noset',
                    sessionname=sessionid,
                    priority=-1,

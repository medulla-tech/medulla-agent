...
import sys
import base64
from  lib.utils import decode_strconsole, encode_strconsole, powerschellscriptps1, simplecommandstr

logger = logging.getLogger()

DEBUGPULSEPLUGIN = 25

plugin = {"VERSION": "1.0", "NAME": "remote_script_monitoring.py", "VERSIONAGENT": "2.0.0", "TYPE": "all"}


def action( objectxmpp, action, sessionid, data, message, dataerreur):
    data_return = { "action": "result" + action,
                    "data": { "result_script" : ""},
                    "sessionid": sessionid,
                    "ret": 255,
                    "base64": False}
    try:
        ...
        if "type_script" in data :
            if data['type_script'].startswith('python'):
                ...
                if sys.platform.startswith('win'):
                    ...
                    obj['code'] = -1
                    obj['result'] = "command dos file .bat"\
                                    "on window only\nplatform is %s" % sys.platform
        else:
        ...
        if obj:
            ...
            data_return['data']['result_script'] = base64.b64encode(zlib.compress(decoderesult, 9))
            data_return['data']['return_code']=obj['code']
            objectxmpp.send_message(mto= message['from'],
                                    mbody=json.dumps(data_return),
                                    mtype='chat')
        else:

...
def action(xmppobject, action, sessionid, data, message, dataerreur):
    ...
    if xmppobject.config.agenttype in ['relayserver']:
        ...
        try:
            ...
            sql = """ SELECT parameter_value FROM guacamole_connection_parameter WHERE connection_id = %s AND parameter_name = 'hostname';""" % (
                data['cux_id'])
            cursor.execute(sql)
            ...
            if hostname != 'localhost':
                ...
                sql = """ SELECT parameter_value FROM guacamole_connection_parameter WHERE connection_id = %s AND parameter_name = 'port';""" % (
                    data['cux_id'])
                cursor.execute(sql)
                results = cursor.fetchall()
        ...
        if data['options'] == "vnclistenmode":
            if sys.platform.startswith('win'):
                try:
                    ...
                    cmd = """\"%s\" -controlservice -disconnectall""" % (
                        program)
                    simplecommand(cmd)
                    cmd = """\"%s\" -controlservice -shareprimary""" % (
                        program)
                    simplecommand(cmd)
                    cmd = """\"%s\" -controlservice -connect localhost""" % (
                        program)
                    simplecommand(cmd)
                except Exception as e:
                    logger.error(
                        "Error start VNC listener TightVNC: %s" %
                        str(e))
                    logger.error("\n%s" % (traceback.format_exc()))
                    raise
            if sys.platform.startswith('darwin'):
                try:
                    ...
                    logger.error(
                        "Error start VNC listener OSXvnc-server: %s" %
                        str(e))
                    logger.error("\n%s" % (traceback.format_exc()))
                    raise
            else:

                    ...
                    if service in active_services:
                        ...
                        try:
                            service_json[service_name]["cpu"] = (
                                int(result_cpu.stdout.readline().split("=")[1])
                                / 1000000000
                            )
                        except ValueError:
                            logger.error(
                                "We set the value to 0 because the systemd"
                ...
                try:
                    ...
                    for share_name in xmppobject.config.shares_list:
                        if share_name == "local":
                            ...
                            share = share_name
                        url = (
                            "http://localhost:8384/rest/db/status?folder=pulsemaster_%s"
                            % share
                        )
            ...
            if xmppobject.config.mysql_enable:
                ...
                try:
                    cnx = mysql.connector.connect(
                        host=xmppobject.config.pulse_main_db_host,
...
def __read_conf_scheduling_mon_pulsesystem(xmppobject):
    ...
    xmppobject.config.ports_list = [
        ...
        "mysql",
        "mmc_agent",
        "pulse2_inventory_server",
        "pulse2_package_server",
    ]
    ...
    if Config.has_section("ports"):
        ...
        if Config.has_option("ports", "pulse2_inventory_server"):
            xmppobject.config.port_pulse2_inventory_server = Config.getint(
                "ports", "pulse2_inventory_server"
            )
        if Config.has_option("ports", "pulse2_package_server"):
            xmppobject.config.port_pulse2_package_server = Config.getint(
                "ports", "pulse2_package_server"
            )
    ...
    if Config.has_section("pulse_relay"):
        if Config.has_option("pulse_relay", "enable"):
            xmppobject.config.pulse_relay_enable = Config.getboolean(
                "pulse_relay", "enable"
            )
    if Config.has_section("pulse_main"):
        if Config.has_option("pulse_main", "enable"):
            xmppobject.config.pulse_main_enable = Config.getboolean(
                "pulse_main", "enable"
            )
        ...
        if Config.has_option("pulse_main", "db_port"):
            xmppobject.config.pulse_main_db_port = Config.getint(
                "pulse_main", "db_port"
            )
        ...
        if Config.has_option("pulse_main", "db_password"):
            xmppobject.config.pulse_main_db_password = Config.get(
                "pulse_main", "db_password"
            )

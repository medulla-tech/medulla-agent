...
class MUCBot(ClientXMPP):
    def __init__(self, conf):  # jid, password, room, nick):
        ...
        conf.jidagent = newjidconf[0] + "@" + \
            resourcejid[0] + "/" + self.HostNameSystem
        ...
        if self.config.syncthing_on:
            ...
            try:
                ...
                confsyncthing = {
                    "action": "resultconfsyncthing", "sessionid": getRandomName(
                        6, "confsyncthing"), "ret": 255, "data": {
                        'errorsyncthingconf': informationerror}}
        ...
        if not self.is_exist_device_in_config(keydevicesyncthing):
            ...
            for dev in self.syncthing.config['devices']:
                ...
                logger.info(
                    ...
                    (dev['deviceID'], dev['name'], json.dumps(
                        dev, indent=4)))
            ...
            if self.session == data['sessionid'] and \
                ...
                if data['ret'] == 0:
                    ...
                    if fromagent == self.sub_assessor:
                        ...
                        if len(data['data']) == 0:
                            logging.error(
                                "Verify table cluster : has_cluster_ars")
                            sys.exit(0)
                        logging.info(
                        ...
                        if self.config.syncthing_on:
                            try:
                                if "syncthing" in data:
                                    self.syncthing.config['options']['globalAnnounceServers'] = [
                                        data["syncthing"]]
                                    ...
                                    if sys.platform.startswith('win'):
                                        defaultFolderPath = "%s\\pulse\\var\\syncthing" % os.environ[
                                            'programfiles']
                                        ...
                                        defaultFolderPath = os.path.join(
                                            "/", "Library", "Application Support", "Pulse", "var", "syncthing")
                                ...
                                if self.deviceid != "":
                                    if len(data['data'][0]) >= 7:
                                        for x in data['data']:
                                            if self.is_format_key_device(
                                                    str(x[5])):
                                                self.adddevicesyncthing(str(x[5]), str(x[2]), address=[
                                                                        "tcp4://%s:%s" % (x[0], x[6])])
                                    logger.debug(
                                        "synchro config %s" %
                                    ...
                                    time.sleep(2)
                                    filesyncthing = os.path.join(os.path.dirname(
                                        os.path.realpath(__file__)), "baseconfigsyncthing.xml")
                                    logging.log(
                                    ...
                                    self.send_message(
                                        mto=msg['from'], mbody=json.dumps(confsyncthing), mtype='chat')
                            except Exception:
                                confsyncthing = {
                                    "action": "resultconfsyncthing",
                                    "sessionid": getRandomName(
                        ...
                        try:
                            if "substitute" in data:
                                ...
                                changeconfigurationsubtitute(conffilenametmp(
                                    opts.typemachine), data['substitute'])
                        except Exception as e:
                        ...
                        try:
                            ...
                            try:
                                ...
                                confaccountclear = {
                                    "action": "resultcleanconfaccount", "sessionid": getRandomName(
                                        6, "delconf"), "ret": 0, "base64": False, "data": {
                                        'useraccount': str(
                                            self.boundjid.user)}}
                                self.send_message(
                                    mto=msg['from'], mbody=json.dumps(confaccountclear), mtype='chat')
                                # go to next ARS
    ...
    def infos_machine_assessor(self):
        ...
        msginfo['data']['codechaine'] = cipher.encrypt(
            str(self.boundjid)).decode()
        self.send_message(mto=self.sub_assessor,
        ...
        try:
            ...
            if 'type' in msgkey:
                ...
                if type == "chat":
                    ...
                    # environment
                    logger.error(
                        "Stanza groupchat message no process %s " %
                        msg)
                    msg.reply(
                        'Thank you, but I do not treat groupchat messages').send()
...
def doTask(
    ...
    while True:
        logging.log(DEBUGPULSE, "ipfromdns %s %s" %
                    (ipfromdns(tg.confserver), tg.confserver))
        ...
        logging.log(
            DEBUGPULSE,
            "verify a information ip or dns for connection configurator")
        if ipfromdns(tg.confserver) == "":
            logging.log(
                DEBUGPULSE,
                "Error while contacting : %s " %
                tg.confserver)
        time.sleep(2)
    if tg.agenttype != "relayserver":
        logging.log(DEBUGPULSE, "connect %s %s" %
        ...
        logging.log(
            DEBUGPULSE, "Warning: A relay server holds a Static "

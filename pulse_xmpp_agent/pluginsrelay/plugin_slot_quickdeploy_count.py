...
plugin = {"VERSION": "1.1", "NAME": "slot_quickdeploy_count", "TYPE": "relayserver"}
        ...
        if data["subaction"] == "restitution":
            try:
                ...
                try:
                    ...
                    objectxmpp.xmpplog( "Deployment message acknowledged\nFreeing quick deployment resource " \
            ...
            while len(objectxmpp.concurrentquickdeployments) < objectxmpp.config.nbconcurrentquickdeployments:
                ...
                try:
                    ...
                    try:
                        ...
                        objectxmpp.concurrentquickdeployments[sessioniddata]=int(time.time())
                    finally:
                        objectxmpp.mutex.release()
...
def timeoutslot(objectxmpp):
    ...
    for delkey in supp:
        ...
        objectxmpp.xmpplog( "Freeing quick deployment resource %s after timeout\n" \
                            "Resource status: %s/%s"%(delkey,
                                                len(objectxmpp.concurrentquickdeployments),
                                                objectxmpp.config.nbconcurrentquickdeployments),
                            type = 'deploy',
                            sessionname = delkey,

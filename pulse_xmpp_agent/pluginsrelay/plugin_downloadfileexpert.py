    ...
    for directory in data['path_src_machine_dir']:
        if reversessh is False:
            if str(data['osmachine']).startswith('Linux'):
                ...
                source = create_path(type = "windows", host = profiluserpulse, ipordomain = data['ipmachine'], path = r'%s'%directory)
        else:
            if str(data['osmachine']).startswith('Linux'):
                source = create_path(type = "linux", host = profiluserpulse, ipordomain="localhost", path = r'%s'%directory)
            elif str(data['osmachine']).startswith('darwin'):
                source = create_path(type = "linux", host = profiluserpulse, ipordomain="localhost", path = r'%s'%directory)
            else:
        ...
        if z['code'] != 0:
            ...
            objectxmpp.xmpplog( 'Copying file %s from machine %s to Master successful'%( os.path.basename(directory), data['hostname']),
                                    type = 'noset',
                                    sessionname = sessionid,
                                    priority = -1,
                                    action = "xmpplog",
            ...
            if z['code'] == 0:
                objectxmpp.xmpplog( 'Transfer result : ' + '\n'.join(z['result']),
                                    ...
                                    fromuser = "",
                                    touser = "")
                objectxmpp.xmpplog( 'Setting mode 777 to file %s '%( os.path.basename(directory)),
                                    type = 'noset',
                                    sessionname = sessionid,
                                    priority = -1,
                                    action = "xmpplog",
                ...
                objectxmpp.xmpplog( 'Error setting mode 777 to file %s : %s'%( os.path.basename(directory), z['result']),
                                    type = 'noset',
                                    sessionname = sessionid,
                                    priority = -1,
                                    action = "xmpplog",

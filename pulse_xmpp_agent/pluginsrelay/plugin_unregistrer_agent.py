...
def action(xmppobject, action, sessionid, data, msg, dataerreur):
    ...
    if "user" in data and "domain" in data and "resource" in data and \
        ...
        try:
            ...
            if res['code'] == 0:
                logger.debug("We correctly removed the account %s@%s" % (data['user'],
                                                                         data['domain']))
            else:
                logger.error("We failed to remove the account %s@%s" % (data['user'],
                                                                        data['domain']))
        except Exception as e:
            logger.error("An error occured while using the unregistrer_agent plugin. We got the error %s" % str(e))
